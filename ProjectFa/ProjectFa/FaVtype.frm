...
Private Const ChqDate As Byte = 10, ClgDate As Byte = 11, DefaultCrAC As Byte = 12, DefaultDrAC As Byte = 13, FirstDrCr As Byte = 14
...
'On Error GoTo ELoop
    ...
    If PubSec = "SANJEEV" Then
        ...
        End If
    ElseIf PubSec = "RAHUL" And pubUName <> "ADMIN" Then
        If rsUserPerm.RecordCount > 0 Then
            rsUserPerm.MoveFirst
...
On Error GoTo ELoop
    ...
    If TopCtrl1.TopText2.CAPTION = "Add" Then
        ...
        Else
            G_FaCn.Execute ("Delete From Voucher_Prefix Where V_Type='" & txt(VType) & "'")
        End If
        ...
        G_FaCn.Execute "Insert Into Voucher_Type(Category,NCat,V_Type,Description,Short_Name,Number_Method,Separate_Narr,Common_Narr,Narration,ChqNo,ChqDt,ClgDt,U_Name,U_EntDt,U_AE,DefaultCrAC,DefaultDrAC,FirstDrCr) Values ('" & txt(Categ) & "','" & txt(NCat) & "','" & txt(VType) & "','" & txt(Desc) & "','" & txt(ShortName) & "','" & txt(VouNumMethod) & "','" & SepNarr1 & "','" & CommNarr1 & "','" & txt(Narration) & "','" & ChqNo1 & "','" & ChqDt1 & "','" & ClgDt1 & "','" & pubUName & "'," & FaConvertDate(PubLoginDate) & ",'A'," & FaChk_Text(txt(DefaultCrAC).Tag) & "," & FaChk_Text(txt(DefaultDrAC).Tag) & "," & FaChk_Text(txt(FirstDrCr)) & ")"
        For I = 1 To FGrid.Rows - 1
            If FGrid.TextMatrix(I, 1) <> "" And FGrid.TextMatrix(I, 2) <> "" And FGrid.TextMatrix(I, 3) <> "" Then
                ...
                Else
                    G_FaCn.Execute ("Insert Into Voucher_Prefix(V_Type,Date_From,Date_To,Prefix,Start_Srl_No) Values ('" & FGrid.TextMatrix(I, FVType) & "'," & FaConvertDate(FGrid.TextMatrix(I, FFromDate)) & "," & FaConvertDate(FGrid.TextMatrix(I, FToDate)) & ",'" & FGrid.TextMatrix(I, FPrefix) & "'," & Val(FGrid.TextMatrix(I, FStartSrNo)) & ")")
                End If
            End If
        Next
    ...
    Select Case Index
        ...
        Case VouNumMethod
            ...
            Else
                KeyAscii = 0
            End If
        Case SepNarr, CommNarr, ChqNo, ChqDate, ClgDate
...
Case 0
    Select Case FGrid.Col
        ...
        Case FFromDate
            ...
            Else
                FGrid.TextMatrix(FGrid.Row, FFromDate) = PubDatamanFa.FaRetDateFunc(TxtGrid(0))
                FGrid.TextMatrix(FGrid.Row, FPrefix) = Year(PubDatamanFa.FaRetDateFunc(TxtGrid(0)))
                FGrid.TextMatrix(FGrid.Row, FVType) = txt(VType)
            End If
        Case FToDate
            If Len(Trim(TxtGrid(0).TEXT)) = 0 Then
            ...
            Else
                FGrid.TextMatrix(FGrid.Row, FToDate) = PubDatamanFa.FaRetDateFunc(TxtGrid(0))
            End If
            FGrid.TextMatrix(FGrid.Row, FVType) = txt(VType)
    End Select
Case 1
...
If Master.RecordCount > 0 Then
    ...
    End If
    If (PubFaSiteType = 1 And PubSeparateVrNoForSite = 1) Then
        Set Rst = G_FaCn.Execute("Select V_Type,Date_From,Date_to,Prefix,Start_Srl_No From Voucher_Prefix Where V_Type='" & Master!V_tYPE & "' AND SITE_CODE='" & PubSeparateLogSite & "' Order By Date_From")
    ElseIf PubFaSiteType = 2 Then
        Set Rst = G_FaCn.Execute("Select V_Type,Date_From,Date_to,Prefix,Start_Srl_No From Voucher_Prefix Where V_Type='" & Master!V_tYPE & "' AND SITE_CODE='" & PubSiteCode & "' Order By Date_From")
    Else
        Set Rst = G_FaCn.Execute("Select V_Type,Date_From,Date_to,Prefix,Start_Srl_No From Voucher_Prefix Where V_Type='" & Master!V_tYPE & "'  Order By Date_From")
    End If
    If Rst.RecordCount > 0 Then
        Do Until Rst.EOF
            FGrid.AddItem ""
...
Private Sub Ini_Grid()
    SSTab1.Tab = 0
    SSTab1.left = 60: SSTab1.top = 3165
