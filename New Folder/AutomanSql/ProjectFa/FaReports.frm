...
Begin VB.Form FaReports 
   ...
   Begin VB.Frame FrDetail 
      ...
      Begin TabDlg.SSTab SSTab1 
         ...
         Tab(1).Control(0)=   "TxtFooter(4)"
         Tab(1).Control(1)=   "TxtFooter(3)"
         Tab(1).Control(2)=   "TxtFooter(2)"
         Tab(1).Control(3)=   "TxtFooter(1)"
         Tab(1).Control(4)=   "TxtFooter(0)"
...
Private Sub DGSite_Click()
    If GRepFormName = "DayBook" Or GRepFormName = "JournalBook" Or GRepFormName = "DUELIST" Then
        ...
        Select Case GRepFormName
          ...
          Case "DUELIST"
            ...
            If RstSite!Code = "" Then
                ...
                Else
                If PubBackEnd = "A" Then
                                 GridInitialise 1, "Select '' as O,SubGroup.Name as PartyName,SubGroup.Subcode as Code,RTRIM(IIF(ISNULL(ADD1) OR LEN(ADD1)=0,'',ADD1))+RTRIM(IIF(ISNULL(ADD2) OR LEN(ADD2)=0,'',','+ADD2))+RTRIM(IIF(ISNULL(CityName) OR LEN(CityName)=0,'',','+CityName)) AS NameWithADDR From SubGroup Left Join City C on SubGroup.CityCode=C.CityCode Where SubGroup.Nature='Customer' AND SUBGROUP.SITE_CODE='" & RstSite!Code & "' Order by SubGroup.Name"
                    ElseIf PubBackEnd = "S" Then
                        GridInitialise 1, "Select '' as O,SubGroup.Name as PartyName,SubGroup.Subcode as Code,RTrim(IsNull(Add1,''))+','+RTrim(IsNull(Add2,''))+','+RTrim(IsNull(CityName,'')) AS NameWithADDR From SubGroup Left Join City C on SubGroup.CityCode=C.CityCode Where SubGroup.Nature='Customer' AND SUBGROUP.SITE_CODE='" & RstSite!Code & "' Order by SubGroup.Name"
                    End If
...
End Sub
Private Sub sitehelpDe()
Select Case GRepFormName
        Case "Led", "LedInt", "LedDeb", "LedCred", "AcCheckList", "DayBook", "CashBook", "BankBook" ', "DUELIST"
          If PubSiteWiseDisplayYn = 1 Then
                If PubSiteType <> "H" Then
                        Label1(6).Visible = True
                        TEXT(SiteCode).Visible = True
                        TEXT(SiteCode).Enabled = False
                    
                        Set RstSite = New ADODB.Recordset
                        RstSite.CursorLocation = adUseClient
                        RstSite.Open "Select Site_Code AS Code,Site_Desc AS NAME From Site where site_code='" & PubSiteCode & "' Order by Site_Desc", G_FaCn, adOpenForwardOnly, adLockReadOnly
                        If RstSite.RecordCount > 0 Then
                           TEXT(SiteCode).Tag = RstSite!Code
                           TEXT(SiteCode).TEXT = RstSite!Name
                        End If
...
Select Case GRepFormName
    ...
    Case "DayBook"
        ...
        If PubSiteWiseDisplayYn = 1 Then
                ...
                If UCase(PubSiteType) <> "H" Then
                    FGrid.TextMatrix(List2, 1) = PubSiteName
                    FGrid.TextMatrix(List2, 2) = PubSiteCode
                    FGrid.RowHeight(List2) = 0
                    mFirstRow = Date1: FGrid.Row = mFirstRow: mLastRow = List1: mHelpGridNo = 0
                Else
                    mFirstRow = Date1: FGrid.Row = mFirstRow: mLastRow = List2: mHelpGridNo = 0
                ...
                If PubSiteWiseDisplayYn = 1 Then
                    If PubSiteType = "H" Then
                      FGrid.TextMatrix(List3, 0) = "For Site": FGrid.RowHeight(List3) = GridRowHeight
                        ...
                        mFirstRow = Date1: FGrid.Row = mFirstRow: mLastRow = List3: mHelpGridNo = 0
                    Else
                        FGrid.TextMatrix(List3, 0) = "For Site": FGrid.RowHeight(List3) = 0
                        FGrid.TextMatrix(List3, 1) = ""
                        Set RstSite = New ADODB.Recordset
                        RstSite.CursorLocation = adUseClient
                        RstSite.Open "Select Site_Code AS Code,Site_Desc AS NAME From Site where site_code='" & PubSiteCode & "' Order by Site_Desc", G_FaCn, adOpenForwardOnly, adLockReadOnly
                        FGrid.TextMatrix(List3, 1) = PubSiteName
                        FGrid.TextMatrix(List3, 2) = PubSiteCode
                        Set DGSite.DataSource = RstSite
                        mFirstRow = Date1: FGrid.Row = mFirstRow: mLastRow = List2: mHelpGridNo = 0
                    End If
                Else
...
'
'                .TextMatrix(List3, 0) = "For Site": .RowHeight(List3) = GridRowHeight
'                .TextMatrix(List3, 1) = ""
        End With
'
'           FGrid.ColWidth(0) = 2500: FGrid.ColWidth(1) = 1500
...
'                    FGrid.TextMatrix(List3, 2) = PubSiteCode
'                    FGrid.RowHeight(List3) = 0
'                    mFirstRow = Date1: FGrid.Row = mFirstRow: mLastRow = List3: mHelpGridNo = 1
'                Else
'                    mFirstRow = Date1: FGrid.Row = mFirstRow: mLastRow = List3: mHelpGridNo = 1
'                End If
'        'Else
...
If Trim(TEXT(SiteCode)) <> "" Then
    If PubFaSiteType = 1 And PubSiteWiseDisplayYn = 1 Then
        mSiteCode = " And RIGHT(LEDGER.Site_Code,1)='" & TEXT(SiteCode).Tag & "'"
...
If FGrid.TextMatrix(List2, 1) = "No" Then
    ...
    If RST1.RecordCount > 0 Then
        ...
        Select Case mNARR1
            ...
            Case "A", "L"
                If PubBackEnd = "A" Then
                    RST1.Open ("SELECT 1 AS TT,MAX(ACGROUP.GROUPNAME)As GroupName,'' AS GrCode,MAX(LEDGER.SUBCODE) AS PARTYCODE,MAX(ViewSubgroup.NAME) AS PARTYNAME,IIF(ISNULL(sum(AMTCR)),0,sum(AMTCR)) AS OP_CR,IIF(ISNULL(SUM(AMTDR)),0,SUM(AMTDR)) AS OP_dR,0 AS BALANCECR,0 AS BALANCEDR,0 As Bal, " & xIsNull("Max(ViewSubGroup.Add1)", "") & " As Add1, " & xIsNull("Max(ViewSubGroup.Add2)", "") & " As Add2, " & xIsNull("Max(ViewSubGroup.CityName)", "") & " As CityName FROM (LEDGER LEFT JOIN ViewSubgroup ON ViewSubgroup.SUBCODE=LEDGER.SUBCODE) LEFT JOIN ACGROUP ON ACGROUP.GROUPCODE=VIEWSUBGROUP.GROUPCODE WHERE V_DATE<" & FaConvertDate(TXTS_DATE) & " " & CondStr1 & " AND ViewSubgroup.GROUPCODE='" & mAcCode & "' AND ACGROUP.AliasYN='N'  " & mSiteCode & " GROUP BY ACGROUP.GROUPNAME,LEDGER.SUBCODE " & _
                    "Union SELECT 2 AS TT,MAX(ACGROUP.GROUPNAME)As GroupName,'' AS GrCode,MAX(LEDGER.SUBCODE) AS PARTYCODE,MAX(ViewSubgroup.NAME) AS PARTYNAME,0 AS OP_CR,0 AS OP_DR,IIF(ISNULL(sum(AMTCR)),0,sum(AMTCR)) AS BALANCECR,IIF(ISNULL(SUM(AMTDR)),0,SUM(AMTDR)) AS BALANCEDR,0 As Bal, " & xIsNull("Max(ViewSubGroup.Add1)", "") & " As Add1, " & xIsNull("Max(ViewSubGroup.Add2)", "") & " As Add2, " & xIsNull("Max(ViewSubGroup.CityName)", "") & " As CityName FROM (LEDGER LEFT JOIN ViewSubgroup ON ViewSubgroup.SUBCODE=LEDGER.SUBCODE) LEFT JOIN ACGROUP ON ACGROUP.GROUPCODE=VIEWSUBGROUP.GROUPCODE WHERE V_DATE BETWEEN " & FaConvertDate(TXTS_DATE) & " AND " & FaConvertDate(TXTE_DATE) & " " & CondStr1 & " AND ViewSubgroup.GROUPCODE='" & mAcCode & "' AND ACGROUP.AliasYN='N'  " & mSiteCode & " GROUP BY ACGROUP.GROUPNAME,LEDGER.SUBCODE " & _
                    "Union  SELECT 3 AS TT,ACGROUP.GROUPNAME,MAX(ACGROUP.GROUPCODE) AS GrCode,'' AS PARTYCODE,ACGROUP.GROUPNAME AS PARTYNAME,IIF(ISNULL(sum(AMTCR)),0,sum(AMTCR)) AS OP_CR,IIF(ISNULL(SUM(AMTDR)),0,SUM(AMTDR)) As OP_DR,0 AS BALANCECR,0 AS BALANCEDR,0 AS Bal, ' '  As add1, ' ' As Add2, ' ' As CityName FROM (ACGROUP INNER JOIN ViewSubgroup ON ACGROUP.MAINGRCODE=" & IIf(PubBackEnd = "S", "SUBSTRING", "MID") & " (ViewSubgroup.MAINGRCODES,1,LEN(ACGROUP.MAINGRCODE))) LEFT JOIN LEDGER ON LEDGER.SUBCODE=VIEWSUBGROUP.SUBCODE WHERE V_DATE<" & FaConvertDate(TXTS_DATE) & " AND ACGROUP.ALIASYN='N' AND ViewSubgroup.AliasYN='N' AND LEFT(MAINGRCODE,LEN('" & mQRY1 & "'))='" & mQRY1 & "' AND LEN(MAINGRCODE)=LEN('" & mQRY1 & "')+3  " & mSiteCode & " GROUP BY ACGROUP.MAINGRCODE,ACGROUP.GROUPNAME " & _
                    "Union  SELECT 4 AS TT,ACGROUP.GROUPNAME,MAX(ACGROUP.GROUPCODE) AS GrCode,'' AS PARTYCODE,ACGROUP.GROUPNAME AS PARTYNAME,0 AS OP_CR,0 AS OP_DR,IIF(ISNULL(sum(AMTCR)),0,sum(AMTCR)) AS BALANCECR,IIF(ISNULL(SUM(AMTDR)),0,SUM(AMTDR)) AS BALANCEDR,0 As Bal, ' '  As add1, ' ' As Add2, ' ' As CityName FROM (ACGROUP INNER JOIN ViewSubgroup ON ACGROUP.MAINGRCODE=" & IIf(PubBackEnd = "S", "SUBSTRING", "MID") & " (ViewSubgroup.MAINGRCODES,1,LEN(ACGROUP.MAINGRCODE))) LEFT JOIN LEDGER ON LEDGER.SUBCODE=VIEWSUBGROUP.SUBCODE WHERE V_DATE BETWEEN " & FaConvertDate(TXTS_DATE) & " AND " & FaConvertDate(TXTE_DATE) & " AND ACGROUP.ALIASYN='N' AND ViewSubgroup.AliasYN='N' AND LEFT(MAINGRCODE,LEN('" & mQRY1 & "'))='" & mQRY1 & "' AND LEN(MAINGRCODE)=LEN('" & mQRY1 & "')+3  " & mSiteCode & " GROUP BY ACGROUP.MAINGRCODE,ACGROUP.GROUPNAME"), G_FaCn, adOpenDynamic, adLockOptimistic
                ElseIf PubBackEnd = "S" Then
                    RST1.Open ("SELECT 1 AS TT,MAX(ACGROUP.GROUPNAME)As GroupName,'' AS GrCode,MAX(LEDGER.SUBCODE) AS PARTYCODE,MAX(ViewSubgroup.NAME)  AS PARTYNAME,ISNULL(sum(AMTCR),0) AS OP_CR,ISNULL(SUM(AMTDR),0) AS OP_dR,0 AS BALANCECR,0 AS BALANCEDR,0 As Bal, IsNull(Max(ViewSubGroup.Add1),'') As Add1, IsNull(Max(ViewSubGroup.Add2),'') As Add2, IsNull(Max(ViewSubGroup.CityName),'') As CityName FROM (LEDGER LEFT JOIN ViewSubgroup ON ViewSubgroup.SUBCODE=LEDGER.SUBCODE) LEFT JOIN ACGROUP ON ACGROUP.GROUPCODE=VIEWSUBGROUP.GROUPCODE WHERE V_DATE<" & FaConvertDate(TXTS_DATE) & " " & CondStr1 & " AND ViewSubgroup.GROUPCODE='" & mAcCode & "' AND ACGROUP.AliasYN='N'  " & mSiteCode & " GROUP BY ACGROUP.GROUPNAME,LEDGER.SUBCODE " & _
                    "Union SELECT 2 AS TT,MAX(ACGROUP.GROUPNAME)As GroupName,'' AS GrCode,MAX(LEDGER.SUBCODE) AS PARTYCODE,MAX(ViewSubgroup.NAME) AS PARTYNAME,0 AS OP_CR,0 AS OP_DR,ISNULL(sum(AMTCR),0) AS BALANCECR,ISNULL(SUM(AMTDR),0) AS BALANCEDR,0 As Bal, IsNull(Max(ViewSubGroup.Add1),'') As Add1, IsNull(Max(ViewSubGroup.Add2),'') As Add2, IsNull(Max(ViewSubGroup.CityName),'') As CityName FROM (LEDGER LEFT JOIN ViewSubgroup ON ViewSubgroup.SUBCODE=LEDGER.SUBCODE) LEFT JOIN ACGROUP ON ACGROUP.GROUPCODE=VIEWSUBGROUP.GROUPCODE WHERE V_DATE BETWEEN " & FaConvertDate(TXTS_DATE) & " AND " & FaConvertDate(TXTE_DATE) & " " & CondStr1 & " AND ViewSubgroup.GROUPCODE='" & mAcCode & "' AND ACGROUP.AliasYN='N'  " & mSiteCode & " GROUP BY ACGROUP.GROUPNAME,LEDGER.SUBCODE " & _
                    "Union  SELECT 3 AS TT,ACGROUP.GROUPNAME,MAX(ACGROUP.GROUPCODE) AS GrCode,'' AS PARTYCODE,ACGROUP.GROUPNAME AS PARTYNAME,ISNULL(SUM(AMTCR),0) AS OP_CR,ISNULL(sum(AMTDR),0) As OP_DR,0 AS BALANCECR,0 AS BALANCEDR,0 AS Bal, ' ' As Add1, ' ' As Add2, ' ' As CityName FROM (ACGROUP INNER JOIN ViewSubgroup ON ACGROUP.MAINGRCODE=" & IIf(PubBackEnd = "S", "SUBSTRING", "MID") & " (ViewSubgroup.MAINGRCODES,1,LEN(ACGROUP.MAINGRCODE))) LEFT JOIN LEDGER ON LEDGER.SUBCODE=VIEWSUBGROUP.SUBCODE WHERE V_DATE<" & FaConvertDate(TXTS_DATE) & " AND ACGROUP.ALIASYN='N' AND ViewSubgroup.AliasYN='N' AND LEFT(MAINGRCODE,LEN('" & mQRY1 & "'))='" & mQRY1 & "' AND LEN(MAINGRCODE)=LEN('" & mQRY1 & "')+3  " & mSiteCode & " GROUP BY ACGROUP.MAINGRCODE,ACGROUP.GROUPNAME " & _
                    "Union  SELECT 4 AS TT,ACGROUP.GROUPNAME,MAX(ACGROUP.GROUPCODE) AS GrCode,'' AS PARTYCODE,ACGROUP.GROUPNAME AS PARTYNAME,0 AS OP_CR,0 AS OP_DR,ISNULL(sum(AMTCR),0) AS BALANCECR,ISNULL(SUM(AMTDR),0) AS BALANCEDR,0 As Bal, ' ' As Add1, ' ' As Add2, ' ' As CityName FROM (ACGROUP INNER JOIN ViewSubgroup ON ACGROUP.MAINGRCODE=" & IIf(PubBackEnd = "S", "SUBSTRING", "MID") & " (ViewSubgroup.MAINGRCODES,1,LEN(ACGROUP.MAINGRCODE))) LEFT JOIN LEDGER ON LEDGER.SUBCODE=VIEWSUBGROUP.SUBCODE WHERE V_DATE BETWEEN " & FaConvertDate(TXTS_DATE) & " AND " & FaConvertDate(TXTE_DATE) & " AND ACGROUP.ALIASYN='N' AND ViewSubgroup.AliasYN='N' AND LEFT(MAINGRCODE,LEN('" & mQRY1 & "'))='" & mQRY1 & "' AND LEN(MAINGRCODE)=LEN('" & mQRY1 & "')+3  " & mSiteCode & " GROUP BY ACGROUP.MAINGRCODE,ACGROUP.GROUPNAME"), G_FaCn, adOpenDynamic, adLockOptimistic
                End If
        End Select
    End If
    If RST1.RecordCount <= 0 Then MsgBox "** No Records Found to Print **", vbInformation, Me.CAPTION: RepPrint = False: Exit Sub
    Select Case Index
        ...
        Case Else
            ...
            CreateFieldDefFile RST1, PubFaReportPath + "\FaANNEXURE.ttx", True
            'Set rpt = PubDatamanFa.FaAnnexureRpt
            Set rpt = rdApp.OpenReport(PubFaReportPath & "\FaANNEXURE.RPT")
...
Dim ARR(7) As Double, Dr1 As Double, DR2 As Double, DR3 As Double, DR4 As Double, DR5 As Double, DR6 As Double, DR7 As Double, DAYS As Integer
Dim X1
If IsNotBlank(Date1, FGrid.TextMatrix(Date1, 0)) = False Then RepPrint = False: Exit Sub
GridString1 = ""
Condstr = ""
...
Do Until SUBGROUP_rs.EOF
    ...
    dr = 0
    cr = 0
    Set mGROUP_rs = G_FaCn.Execute("SELECT V_DATE,DEBIT,CREDIT,PARTY AS SUBCODE FROM VIEWLEDGER WHERE '" & SUBGROUP_rs!SubCode & "'=VIEWLEDGER.PARTY AND V_DATE<=" & FaConvertDate(TXTE_DATE))
...
'Set rpt = rdApp.OpenReport(PubRepoPath + "\FaAgeing.RPT")
For I = 1 To rpt.FormulaFields.Count
...
mAcCode = Trim(FGrid.TextMatrix(List1, 2))
If Trim(FGrid.TextMatrix(List3, 2)) <> "" Then
    If PubFaSiteType = 1 Or PubSiteWiseDisplayYn = 1 Then
        ...
        Case 1
            Select Case mType
                Case "Led", "AcCheckList"
                    ...
                    LedDos Me, TmpRst, mSepratePage, TEXT(PrintVrNo)
                    GoTo ExitLoop
                    Exit Sub
                Case "LedInt"
...
For I = 1 To rpt.FormulaFields.Count
    Select Case rpt.FormulaFields(I).FormulaFieldName
        ...
        Case "END_DATE"
            If mType = "LedInt" Then rpt.FormulaFields(I).TEXT = "DATE(" & ConvertDate(Format(CDate(TXTE_DATE), "YYYY,MM,DD")) & ")"
        Case "SEPRATEPAGE"
...
Public Sub FillTmp_LedIndex(Rst As ADODB.Recordset)
    Set Rst = New ADODB.Recordset
    With Rst
        .Fields.Append "PARTYNAME", adVarChar, 255, adFldIsNullable
        .Fields.Append "Address", adVarChar, 255, adFldIsNullable
        .Fields.Append "PageNo", adDouble, , adFldIsNullable
        .CursorType = adOpenStatic
        .LockType = adLockOptimistic
        .Open
    End With
...
Public Sub LedDos(FormName As Object, RstRowSource As ADODB.Recordset, SeparatePage As String, PrintVrNo As String)
On Error GoTo ELoop
Dim RsIndex As ADODB.Recordset
Dim mROW As Integer, mPAGE As Integer, PrintLine As String, RstEnvi As ADODB.Recordset
Dim mDR As Double, mCR As Double, mBAL As Double, mOBAL As Double, mPartyName As String
Dim mFlagEj As Boolean
mROW = 0
mPAGE = 1
mDR = 0
mCR = 0
mBAL = 0
FillTmp_LedIndex RsIndex
...
PrintLine = String(136, LineFil)
RstRowSource.Sort = "NAME1 ASC,PDate ASC,VAL ASC,DOCID ASC"
RstRowSource.MoveFirst
Do Until RstRowSource.EOF
    If mROW = 0 Then
        mROW = FaPRNTIT(mPAGE, mROW, 80)
        If PubFaSiteType <> 0 And FormName.TEXT(12) <> "" Then
            Print #1, FaPRN_TIT(FormName.CAPTION + " (" + FormName.TEXT(12) + ")", "B", 80)
        Else
            Print #1, FaPRN_TIT(FormName.CAPTION, "B", 80)
        End If
        If SeparatePage = "Y" Then
            Print #1, Chr(27) + Chr(71) + FaXNull(RstRowSource!Name1) + Chr(27) + Chr(72): mROW = mROW + 1
            If Len(Trim(Replace(FaXNull(RstRowSource!CITY_NAME), ",", ""))) > 0 Then Print #1, Chr(18) + FaXNull(RstRowSource!Address1): mROW = mROW + 1
            If Len(Trim(FaXNull(RstRowSource!CITY_NAME))) > 0 Then Print #1, Chr(18) + FaXNull(RstRowSource!CITY_NAME): mROW = mROW + 1
        End If
        Print #1, "From Date : " & FormName.TXTS_DATE & " To " & FormName.TXTE_DATE
        Print #1, Chr(15) + PrintLine + Chr(18)
        If PrintVrNo = "Yes" Then
            Print #1, Chr(15) + "Vr.Date     Vr.No.                 Particulars                                                  Debit          Credit         Balance" + Chr(18)
        Else
            Print #1, Chr(15) + "Vr.Date                            Particulars                                                  Debit          Credit         Balance" + Chr(18)
        End If
        Print #1, Chr(15) + PrintLine + Chr(18)
        mROW = mROW + 5
    End If
    If mPartyName <> RstRowSource!Name1 Then
        mDR = 0
        mCR = 0
        mBAL = 0
        mPartyName = RstRowSource!Name1
        If SeparatePage = "N" Then
            Print #1, Chr(27) + Chr(71) + FaXNull(RstRowSource!Name1) + Chr(27) + Chr(72): mROW = mROW + 1
            If Len(Trim(Replace(FaXNull(RstRowSource!CITY_NAME), ",", ""))) > 0 Then Print #1, Chr(18) + FaXNull(RstRowSource!Address1): mROW = mROW + 1
            If Len(Trim(FaXNull(RstRowSource!CITY_NAME))) > 0 Then Print #1, Chr(18) + FaXNull(RstRowSource!CITY_NAME): mROW = mROW + 1
            RsIndex.AddNew
            RsIndex!PartyName = left(FaXNull(RstRowSource!Name1), 255)
            RsIndex!Address = left(FaXNull(RstRowSource!Address1) & ", " & FaXNull(RstRowSource!CITY_NAME), 255)
            RsIndex!PageNo = mPAGE
            RsIndex.Update
        End If
    End If
    mDR = mDR + FaVNull(RstRowSource!ADJAMT)
    mCR = mCR + FaVNull(RstRowSource!cr)
    mBAL = mBAL + (FaVNull(RstRowSource!cr) - FaVNull(RstRowSource!ADJAMT))
    Print #1, Chr(15) + FaSetW(FaXNull(Format(RstRowSource!V_DATE, "dd/MM/yy")), 10) + "  " + FaSetW(FaXNull(IIf(PrintVrNo = "Yes", RstRowSource!DOCNO, "")), 21) + "  " + FaSetW(FaXNull(RstRowSource!Name), 50) + FaSetN(FaBNull(RstRowSource!ADJAMT), 16) + FaSetN(FaBNull(RstRowSource!cr), 16) + IIf(IsDate(RstRowSource!V_DATE), FaSetN(CStr(FaBNull(Abs(mBAL))), 16) + " " + IIf(mBAL > 0, "Cr", IIf(mBAL < 0, "Dr", "  ")), "") + Chr(18)
    mROW = mROW + 1
    mFlagEj = False
    If mROW >= 58 Then
        Print #1, Chr(15) + PrintLine + Chr(18)
        Print #1, Chr(15) + FaSetW("Total", 85) + FaSetN(FaBNull(mDR), 16) + FaSetN(FaBNull(mCR), 16) + Chr(18)
        Print #1, Chr(15) + PrintLine + Chr(18)
        Print #1, Chr(12)
        mPAGE = mPAGE + 1
        mROW = 0
    ...
    If RstRowSource.EOF = True Then
        GoTo LastRecord
    Else
        If mPartyName <> FaXNull(RstRowSource!Name1) And mFlagEj = False Then
LastRecord:
            Print #1, Chr(15) + PrintLine + Chr(18)
            Print #1, Chr(15) + FaSetW("Total", 85) + FaSetN(FaBNull(mDR), 16) + FaSetN(FaBNull(mCR), 16) + Chr(18)
            Print #1, Chr(15) + PrintLine + Chr(18)
            Print #1, Chr(14) + Chr(15) + "Net Balance : " + FaSetN(IIf(Abs(FaVNull(mDR) - FaVNull(mCR)) <> 0, Round(Abs(FaVNull(mDR) - FaVNull(mCR)), 2), ""), 18) + IIf(FaVNull(mDR) - FaVNull(mCR) > 0, " Dr", IIf(FaVNull(mDR) - FaVNull(mCR) < 0, " Cr", "Nil")) + Chr(18)
            mROW = mROW + 4
            If SeparatePage = "Y" Then
                Print #1, Chr(12)
                mPAGE = 1
...
If RsIndex.RecordCount > 0 Then
    If MsgBox("Do You Want to Print Index", vbYesNo) = vbYes Then
        DosPrint_LedIndex RsIndex
...
Public Sub DosPrint_LedIndex(RstRowSource As ADODB.Recordset)
On Error GoTo ELoop

Dim mROW As Integer, mPAGE As Integer, PrintLine As String, RstEnvi As ADODB.Recordset
Dim mDR As Double, mCR As Double, mBAL As Double, mOBAL As Double, mPartyName As String
Dim mFlagEj As Boolean, I As Long
mROW = 0
mPAGE = 1
mDR = 0
mCR = 0
...
Open "C:\reptmp\TEXTFILE.TXT" For Output As #1
PrintLine = String(150, "-")

RstRowSource.MoveFirst
Do Until RstRowSource.EOF
    If mROW = 0 Then
        mROW = FaPRNTIT(mPAGE, mROW, 80)
        Print #1, FaPRN_TIT("INDEX", "B", 80)
        Print #1, Chr(15) + PrintLine + Chr(18)
        Print #1, Space(2) + Chr(27) + Chr(69); FaSetW("Srl", 5) + FaSetW("Ledger Name", 70) + FaSetN("Page No", 6) + Chr(27) + Chr(70)
        Print #1, Chr(15) + PrintLine + Chr(18)
        mROW = mROW + 6
    End If
    'Print #1, Chr(27) + Chr(71) + FaXNull(RstRowSource!Name1) + Chr(27) + Chr(72): mROW = mROW + 1
    Print #1, Space(2) + FaSetW(CStr(I), 5) + FaSetW(FaXNull(RstRowSource!PartyName), 70) + FaSetN(RstRowSource!PageNo, 6): mROW = mROW + 1
    Print #1, Space(7) + FaSetW(FaXNull(RstRowSource!Address), 70): mROW = mROW + 1
    I = I + 1
    mROW = mROW + 1
    mFlagEj = False
    If mROW >= 90 Then
    ...
    If RstRowSource.EOF = True Then
        GoTo LastRecord
    Else
        If mPartyName <> FaXNull(RstRowSource!PartyName) And mFlagEj = False Then
LastRecord:
'            Print #1, Chr(15) + PrintLine + Chr(18)
