...
On Error GoTo Errloop
    ...
    transFlag = 1
    If ADDFLAG = 1 Then
        G_FaCn.Execute "Insert Into TDSCAT (TDS_Catg, TDS_Desc, code,name,TDSLimit,TDSPercentage, Site_Code,U_Name,U_EntDt,U_AE) Values('" & mCode & "','" & txt(Name1).TEXT & "','" & mCode & "','" & txt(Name1).TEXT & "'," & Val(txt(TDSLimit)) & "," & Val(txt(TDSPercentage)) & ", '" & PubSiteCode & "','" & pubUName & "'," & FaConvertDate(PubLoginDate) & ",'A' )"
    Else
...
Select Case Index
    ...
    Case TDSPercentage
        FaNumPress txt(Index), keyascii, 2, 2
End Select
End Sub
