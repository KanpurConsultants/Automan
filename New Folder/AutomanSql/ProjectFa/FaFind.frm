...
If Master.RecordCount > 0 Then
    For aa = 1 To Master.Fields.Count - 1
        Debug.Print Master.Fields(aa).Name
        Debug.Print Master.Fields(aa).Type
        Debug.Print Master.Fields(aa).DefinedSize
        If Master.Fields(aa).ActualSize = 0 Then
            'GridSel.ColWidth(aa) = Master.Fields(aa).DefinedSize * 120
            GridSel.ColWidth(aa) = 1000
        Else
            If Master.Fields(aa).DefinedSize = 255 Then
                GridSel.ColWidth(aa) = (Master.Fields(aa).ActualSize * 120) + 150
            Else
                GridSel.ColWidth(aa) = (Master.Fields(aa).DefinedSize * 120) + 150
            End If
            'GridSel.ColWidth(aa) = (Master.Fields(aa).ActualSize * 120) + 100
        End If
    Next
End If
