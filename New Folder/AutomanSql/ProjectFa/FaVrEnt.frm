...
Begin VB.Form FaVrEnt 
   ...
   ScaleHeight     =   8580
   ScaleWidth      =   11415
   Begin VB.CommandButton CmdChqPrint 
      Caption         =   "Chq Print"
      BeginProperty Font 
         Name            =   "Tahoma"
         Size            =   9
      ...
      Width           =   1365
   End
   Begin VB.Frame FrameChqDetail 
      BackColor       =   &H00C9E7E0&
      Caption         =   "Cheque Printing Detail"
      BeginProperty Font 
         Name            =   "Tahoma"
         Size            =   9.75
         Charset         =   0
      ...
      Visible         =   0   'False
      Width           =   8475
      Begin VB.CommandButton CmdChqOk 
         Caption         =   "OK"
         BeginProperty Font 
            Name            =   "Tahoma"
            Size            =   9
         ...
         Width           =   1365
      End
      Begin VB.TextBox TxtChq_AcPayee 
         BeginProperty Font 
            Name            =   "Tahoma"
            Size            =   9.75
            Charset         =   0
         ...
         Width           =   1665
      End
      Begin VB.TextBox TxtChq_Favour 
         BeginProperty Font 
            Name            =   "Tahoma"
            Size            =   9.75
            Charset         =   0
...
Dim mAddBy$, mAddDate$, mModifyBy$, mModifyDate$
...
On Error GoTo erroloop
  Dim sitecond As String
    If PubSiteWiseDisplayYn = 1 And UCase(PubSiteType) <> "H" Then
      sitecond = "and  " & cMID("ledger.Docid", "3", "1") & "='" & PubSiteCode & "'"
    Else
      sitecond = ""
    End If
    ...
    FGVLIST.Rows = 1
    mQry = " WHERE NCAT<>'OPBAL' And Voucher_Type.Category='FA'"
    ...
    If PubSiteCodeWiseMasterRst = False Then
        Rst.Open "SELECT LEDGER.*,SUBGROUP.NAME FROM (LEDGER LEFT JOIN SUBGROUP ON SUBGROUP.SUBCODE=LEDGER.SUBCODE) LEFT JOIN VOUCHER_tYPE ON VOUCHER_tYPE.V_TYPE=LEDGER.V_tYPE " & mQry & " " & sitecond & " ORDER BY V_DATE,LEDGER.V_TYPE,V_NO,V_SNo", G_FaCn, adOpenDynamic, adLockOptimistic
    Else
        If (PubFaSiteType = 1 And PubSeparateVrNoForSite = 1) Then
            Rst.Open "SELECT LEDGER.*,SUBGROUP.NAME FROM (LEDGER LEFT JOIN SUBGROUP ON SUBGROUP.SUBCODE=LEDGER.SUBCODE) LEFT JOIN VOUCHER_tYPE ON VOUCHER_tYPE.V_TYPE=LEDGER.V_tYPE " & mQry & " " & sitecond & "  AND RIGHT(LEDGER.SITE_CODE,1)='" & Trim(PubSeparateLogSite) & "' ORDER BY V_DATE,LEDGER.V_TYPE,V_NO,V_SNo", G_FaCn, adOpenDynamic, adLockOptimistic
        ElseIf PubFaSiteType = 1 Then
            Rst.Open "SELECT LEDGER.*,SUBGROUP.NAME FROM (LEDGER LEFT JOIN SUBGROUP ON SUBGROUP.SUBCODE=LEDGER.SUBCODE) LEFT JOIN VOUCHER_tYPE ON VOUCHER_tYPE.V_TYPE=LEDGER.V_tYPE " & mQry & " " & sitecond & " AND LEFT(LEDGER.SITE_CODE,1)='" & Trim(PubSiteCode) & "' ORDER BY V_DATE,LEDGER.V_TYPE,V_NO,V_SNo", G_FaCn, adOpenDynamic, adLockOptimistic
        ElseIf PubFaSiteType = 2 Then
            Rst.Open "SELECT LEDGER.*,SUBGROUP.NAME FROM (LEDGER LEFT JOIN SUBGROUP ON SUBGROUP.SUBCODE=LEDGER.SUBCODE) LEFT JOIN VOUCHER_tYPE ON VOUCHER_tYPE.V_TYPE=LEDGER.V_tYPE " & mQry & "  " & sitecond & " AND LEDGER.SITE_CODE='" & Trim(PubSiteCode) & "'  ORDER BY V_DATE,LEDGER.V_TYPE,V_NO,V_SNo", G_FaCn, adOpenDynamic, adLockOptimistic
        Else
            Rst.Open "SELECT LEDGER.*,SUBGROUP.NAME FROM (LEDGER LEFT JOIN SUBGROUP ON SUBGROUP.SUBCODE=LEDGER.SUBCODE) LEFT JOIN VOUCHER_tYPE ON VOUCHER_tYPE.V_TYPE=LEDGER.V_tYPE " & mQry & " " & sitecond & "  ORDER BY V_DATE,LEDGER.V_TYPE,V_NO,V_SNo", G_FaCn, adOpenDynamic, adLockOptimistic
        End If
    End If
...
Private Sub CmdChqOk_Click()
    FrameChqDetail.Visible = False
End Sub

Private Sub CmdChqPrint_Click()
    If MsgBox("Do you want to See Print on Screen?", vbYesNo) = vbYes Then
        Call WindowsPrint(0, GSQL, True)
    Else
        Call WindowsPrint(1, GSQL, False)
    End If
End Sub
    ...
    If PubBackEnd = "A" Then
        If RstEnviro!FilterAC = "No" Then
            ...
            If RstEnviro!ShowCityName = "Yes" Then
                RstAcHlpDr.Open ("Select SubCode,NameWithCity AS Name,GNAME AS GROUPNAME,GroupCode,MAINGRCODES, RTRIM(" & cIIF("ADD1 Is Null OR LEN(ADD1)=0", "''", "ADD1") & ")+RTRIM(" & cIIF("ADD2 Is Null OR LEN(ADD2)=0", "''", "','+ADD2") & ")+RTRIM(" & cIIF("ADD3 Is Null OR LEN(ADD3)=0", "''", "','+ADD3") & ")+RTRIM(" & cIIF("CityName Is Null", "''", "','+CityName") & ") AS NameWithADDR, " & xIsNull("FNAME", "") & " AS FatherName From ViewSubgroup " & mSiteHlp & " order by Name"), G_FaCn, adOpenKeyset, adLockOptimistic
            Else
...
If KeyCode = vbKeyH And Shift = 2 Then ShowFrameChqDetail
mCancel = False
...
Sub ShowFrameChqDetail()
    If Trim(TxtCHno(0)) <> "" Then
        FrameChqDetail.top = 2000
        FrameChqDetail.left = 2000
        FrameChqDetail.Visible = True
        TxtChq_Favour.SetFocus
    End If
End Sub
...
End If
    ...
    PubUParam = MDIForm1.Permission("Voucher Entry")
...
''    End If
    ...
    Set RstMain = New ADODB.Recordset
      ...
      Dim sitecond As String
    If PubSiteWiseDisplayYn = 1 And UCase(PubSiteType) <> "H" Then
      sitecond = "and  " & cMID("Docid", "3", "1") & "='" & PubSiteCode & "'"
    Else
      sitecond = ""
    End If
    ...
    If PubSiteCodeWiseMasterRst = False Then
        If PubFaSiteType = 1 Then
            ...
            RstMain.Open "SELECT DOCID FROM LEDGERM WHERE V_TYPE IN (SELECT V_TYPE FROM VOUCHER_tYPE WHERE Category='FA') AND V_DATE BETWEEN " & FaConvertDate(PubStartDate) & " AND " & FaConvertDate(PubEndDate) & " " & sitecond & " ORDER BY V_Date,DOCID", G_FaCn, adOpenDynamic, adLockOptimistic
        Else
    ...
    Else
        If PubFaSiteType = 1 Then
            If PubSeparateVrNoForSite = 1 Then
                ...
                RstMain.Open "SELECT DOCID FROM LEDGERM WHERE V_TYPE IN (SELECT V_TYPE FROM VOUCHER_tYPE WHERE Category='FA') AND V_DATE BETWEEN " & FaConvertDate(PubStartDate) & " AND " & FaConvertDate(PubEndDate) & " AND RIGHT(SITE_CODE,1)='" & Trim(PubSeparateLogSite) & "' " & sitecond & " ORDER BY V_Date,DOCID", G_FaCn, adOpenDynamic, adLockOptimistic
            Else
                Label18.Visible = True
                TxtSite.Visible = True
                RstMain.Open "SELECT DOCID FROM LEDGERM WHERE V_TYPE IN (SELECT V_TYPE FROM VOUCHER_tYPE WHERE Category='FA') AND V_DATE BETWEEN " & FaConvertDate(PubStartDate) & " AND " & FaConvertDate(PubEndDate) & "  " & sitecond & " ORDER BY V_Date,DOCID", G_FaCn, adOpenDynamic, adLockOptimistic
        ...
        ElseIf PubFaSiteType = 2 Then
            Label18.Visible = True
            TxtSite.Visible = True
            RstMain.Open "SELECT DOCID FROM LEDGERM WHERE V_TYPE IN (SELECT V_TYPE FROM VOUCHER_tYPE WHERE Category='FA') AND V_DATE BETWEEN " & FaConvertDate(PubStartDate) & " AND " & FaConvertDate(PubEndDate) & "  AND SITE_CODE='" & Trim(PubSiteCode) & "' " & sitecond & " ORDER BY V_Date,DOCID", G_FaCn, adOpenDynamic, adLockOptimistic
        Else
            Label18.Visible = False
            TxtSite.Visible = False
            RstMain.Open "SELECT DOCID FROM LEDGERM WHERE V_TYPE IN (SELECT V_TYPE FROM VOUCHER_tYPE WHERE Category='FA') AND V_DATE BETWEEN " & FaConvertDate(PubStartDate) & " AND " & FaConvertDate(PubEndDate) & "  " & sitecond & " ORDER BY V_Date,DOCID", G_FaCn, adOpenDynamic, adLockOptimistic
...
On Error GoTo ErrLoop
    ...
    Else
        ...
        If RstEnviro!ShowCityName = "Yes" Then
            MyRs.Open "SELECT LEDGER.*, (Select Max(Clg_Date) from Ledger L where L.docId=Ledger.DocID ) as MaxClgDate,NameWithCity AS Name,ViewSubgroup.CURR_BAL FROM (LEDGER LEFT JOIN ViewSubgroup ON ViewSubgroup.SUBCODE=LEDGER.SUBCODE) LEFT JOIN VOUCHER_TYPE ON VOUCHER_TYPE.V_tYPE=LEDGER.V_TYPE WHERE LEDGER.DOCID='" & RstMain!DocID & "' ORDER BY LEDGER.V_SNO", G_FaCn, adOpenForwardOnly, adLockReadOnly
        Else
            MyRs.Open "SELECT LEDGER.*, (Select Max(Clg_Date) from Ledger L where L.docId=Ledger.DocID ) as MaxClgDate,SUBGROUP.NAME,SUBGROUP.CURR_BAL FROM (LEDGER LEFT JOIN SUBGROUP ON SUBGROUP.SUBCODE=LEDGER.SUBCODE) LEFT JOIN VOUCHER_TYPE ON VOUCHER_TYPE.V_tYPE=LEDGER.V_TYPE WHERE LEDGER.DOCID='" & RstMain!DocID & "' ORDER BY LEDGER.V_SNO", G_FaCn, adOpenForwardOnly, adLockReadOnly
        End If
        If MyRs.RecordCount > 0 Then
            ...
            TXTClrDate(0) = FaXNull(MyRs!MaxClgDate)
            TxtChq_Favour = FaXNull(MyRs!Chq_Favour)
            TxtChq_AcPayee = IIf(FaVNull(MyRs!Chq_AcPayee) = 1, "Yes", "No")
            
            If TxtChq_Favour <> "" Then
                CmdChqPrint.Visible = True
            Else
                CmdChqPrint.Visible = False
            End If
            ...
            mAddBy = XNull(MyRs!AddBy)
            mAddDate = XNull(MyRs!AddDate)
            mModifyBy = XNull(MyRs!ModifyBy)
            mModifyDate = XNull(MyRs!ModifyDate)
            
            If UCase(pubUName) = "SA" Or PubULabel = "Y" Then
                LblUser = IIf(Not IsNull(MyRs!AddDate), "Add By : " & XNull(MyRs!AddBy) & "  Dated : " & XNull(MyRs!AddDate), "") & IIf(Not IsNull(MyRs!ModifyDate), "     Modify By : " & XNull(MyRs!ModifyBy) & "  Dated : " & XNull(MyRs!ModifyDate), "")
            Else
                LblUser = ""
            End If
...
If IsDmsVoucher(RstMain!DocID) Then
    MsgBox "This Voucher is Imported From Siebel"
End If
...
Dim CanEditDmsVoucher As Boolean
On Error GoTo ErrLoop

    If IsDmsVoucher(RstMain!DocID) Then
        If MsgBox("This voucher is imported from Siebel. Do u want to delete it?", vbYesNo) = vbYes Then
            CanEditDmsVoucher = True
        Else
            Exit Sub
        End If
    End If
    ...
    If RstMain.RecordCount > 0 Then
        If MsgBox("Are sure to delete it", vbYesNo + vbCritical + vbDefaultButton1, Me.CAPTION) = vbYes Then
            ...
            CreateLog Me, RstMain!DocID, False
            
            FaDeleteTrack G_FaCn, RstMain!DocID
            On Error GoTo ErrLoop
            If CanEditDmsVoucher Then GCn.Execute "Delete from DmsData Where DocId='" & RstMain!DocID & "'"
...
On Error GoTo ErrLoop
    If IsEditable(RetDate(VchDt(0))) = False Then Exit Sub
...
Public Sub Disp_Text(Enb As Boolean)
    '''
    If UCase(left(PubComp_Name, 4)) = "ENAR" Then TxtSite.Enabled = False
    TxtChq_Favour.Enabled = Enb
    TxtChq_AcPayee.Enabled = Enb
    FrameChqDetail.Visible = False
End Sub
...
Dim I As Integer, mDR As Double, mCR As Double, j As Long, mDocId As String, Begin As Byte
...
On Error GoTo err
    ...
    End If
    If AED = "A" Then If G_FaCn.Execute("SELECT COUNT(*) FROM LEDGERM WHERE Left(DocId,1)='" & PubDivCode & "' And V_Type='" & FaSetW(TxtVtYpe(0).Tag, 5) & "' And V_Prefix = '" & LblVPrefix & "' And V_No = " & Val(FaSetN(TxtVno(0), 8)) & " ").Fields(0) > 0 Then LedPost = 2: Exit Function
    ...
    If AED = "E" Then CreateLog Me, RstMain!DocID, False
    If AED = "E" Or AED = "D" Then
        ...
        mChqClrDate = TXTClrDate(0) 'XNull(RST1!Clg_Date)
    ...
    If AED = "A" Or AED = "E" Then
        ...
        For I = 0 To FGrid1.Rows - 1
            If Val(FGrid1.TextMatrix(I, 6)) + Val(FGrid1.TextMatrix(I, 5)) > 0 Then
                ...
                If AED = "A" Then
                    mAddBy = pubUName
                    mAddDate = PubServerDate
                    mModifyBy = ""
                    mModifyDate = ""
                Else
                    mModifyBy = pubUName
                    mModifyDate = PubServerDate
                End If
                
                G_FaCn.Execute "INSERT INTO LEDGER (DocId,V_SNo,V_Type,V_No,v_Prefix,Site_Code,V_Date,SubCode,AmtCr,AmtDr,ContraSub,Narration,U_Name,U_EntDt,U_AE, AddBy, AddDate, ModifyBy, ModifyDate,Chq_No,Chq_Date,Clg_Date,AgRefNo, Chq_Favour, Chq_AcPayee) " & _
                               " VALUES ('" & mDocId & "'," & Val(FGrid1.TextMatrix(I, 1)) & ",'" & TxtVtYpe(0).Tag & "'," & Val(TxtVno(0).TEXT) & "," & FaChk_Text(LblVPrefix) & ",'" & mSanSite & "'," & FaConvertDate(VchDt(0)) & ",'" & FGrid1.TextMatrix(I, 2) & "'," & Val(FGrid1.TextMatrix(I, 6)) & "," & Val(FGrid1.TextMatrix(I, 5)) & "," & FaChk_Text(FGrid1.TextMatrix(I, 4)) & "," & FaChk_Text(FGrid1.TextMatrix(I, 8)) & ",'" & pubUName & "'," & FaConvertDate(Now) & ",'" & AED & "', '" & mAddBy & "', " & ConvertDateTime(mAddDate) & ", '" & mModifyBy & "', " & ConvertDateTime(mModifyDate) & "," & FaChk_Text(Trim(TxtCHno(0))) & "," & FaConvertDate(TXTChDate(0)) & "," & FaConvertDate(TXTClrDate(0)) & "," & FaChk_Text(mRefNo) & ", " & FaChk_Text(TxtChq_Favour) & ", '" & IIf(StrCmp(TxtChq_AcPayee, "Yes"), 1, 0) & "')"
            ...
            End If
        Next
        G_FaCn.Execute "Update Ledger Set Clg_Date=" & ConvertDate(mChqClrDate) & " Where DocId='" & mDocId & "'"
        ...
        Else
            If AED = "A" Then
                G_FaCn.Execute "INSERT INTO LedgerM (DocId,V_Type,v_Prefix,V_No,Site_Code,V_Date,Narration,U_Name,U_EntDt,U_AE) VALUES ('" & mDocId & "','" & TxtVtYpe(0).Tag & "'," & FaChk_Text(LblVPrefix) & "," & Val(TxtVno(0).TEXT) & ",'" & mSanSite & "'," & FaConvertDate(VchDt(0)) & "," & FaChk_Text(Replace(TxtGlb(0), vbCrLf, "")) & ",'" & pubUName & "','" & Format(Now, "dd/MMM/yyyy") & "'," & FaChk_Text(AED) & ")"
                G_FaCn.Execute "UPDATE VOUCHER_Prefix SET Start_Srl_No=" & Val(TxtVno(0).TEXT) & " WHERE V_Type='" & TxtVtYpe(0).Tag & "' and Prefix='" & LblVPrefix & "' aND (Site_Code = '" & PubSiteCode & "' Or IsNull(Site_Code,'')='') "
                mxLastVrType = TxtVtYpe(0).Tag
                mLastPrefix = LblVPrefix
            Else
                G_FaCn.Execute "UPDATE LedgerM SET DocId='" & mDocId & "',V_Type='" & TxtVtYpe(0).Tag & "',v_Prefix=" & FaChk_Text(LblVPrefix) & ",V_No=" & Val(TxtVno(0).TEXT) & ",Site_Code='" & mSanSite & "',V_Date=" & FaConvertDate(VchDt(0)) & ",Narration=" & FaChk_Text(Replace(TxtGlb(0), vbCrLf, "")) & ",U_Name='" & pubUName & "',U_EntDt='" & Format(Now, "dd/MMM/yyyy") & "',U_AE='" & AED & "' WHERE DocId='" & RstMain!DocID & "'"
        ...
        '''''''''''''INSERT RECORD IN PAYMENT TABLE FOR CHEQUE REGISTER'''''''''''''''
        
        
        Dim mQry As String
        
        mQry = "DELETE FROM PAYMENT WHERE DocId = '" & mDocId & "'"
        GCn.Execute mQry
        ...
        mQry = "INSERT INTO Payment ( DocId, Site_Code, V_Date, V_Type, V_No, PartyCode, Amount, AcCode, Chq_No, " & _
                ...
                " VALUES  ( '" & mDocId & "', '" & mSanSite & "', " & FaConvertDate(VchDt(0)) & "," & _
                " '" & TxtVtYpe(0).Tag & "', " & Val(TxtVno(0).TEXT) & ", '', " & Val(LblDrAmt(0)) & "," & _
                " '', " & FaChk_Text(Trim(TxtCHno(0))) & ", " & FaConvertDate(TXTChDate(0)) & ", " & FaConvertDate(TXTClrDate(0)) & "," & _
                " '" & TXTNARRATION & "', '" & TxtChq_Favour & "', '', " & IIf(StrCmp(TxtChq_AcPayee, "Yes"), 1, 0) & "," & _
                " '" & pubUName & "'," & ConvertDate(PubServerDate) & ",'A','',NULL," & _
...
Private Sub btnPrint1_Click()
    ...
    If Opt2(0) Then
        ...
        Else
            Set rpt = rdApp.OpenReport(PubFaReportPath + "\FaJVCHR.RPT")
            FaVoucherPrintingModule Me, rpt, XNull(RstMain!DocID)
        End If
        Set rpt = Nothing
    Else
        If Opt2(2) Then
            If FaIsValid(Text2, "For Date ") = False Then Exit Sub
            Set RsTemp = G_FaCn.Execute("Select DocID from LedgerM Where V_Date=" & ConvertDate(Text2) & "")
        ElseIf Opt2(1) Then
            If FaIsValid(DataCombo3, "Voucher Type ") = False Then Exit Sub
            If FaIsValid(DataCombo2(0), "From Voucher No. ") = False Then Exit Sub
            If FaIsValid(DataCombo2(1), "To Voucher No. ") = False Then Exit Sub
        
            Set RsTemp = G_FaCn.Execute("Select DocID from LedgerM Where Left(DocId,1)='" & PubDivCode & "' and  V_Type='" & DataCombo3.BoundText & "' And V_No Between " & DataCombo2(0) & " and " & DataCombo2(1) & " Order By V_No ")
        Else
            Set RsTemp = G_FaCn.Execute("Select DocID from LedgerM Where DocId='" & RstMain!DocID & "' Order By DocId ")
        End If
        If RsTemp.RecordCount > 0 Then
            Do Until RsTemp.EOF
                
                Call PrintVoucherPlain(RsTemp!DocID)
                
                If RsTemp.RecordCount > 0 Then Delay 10
                RsTemp.MoveNext
            Loop
        Else
...
Private Sub TxtChq_AcPayee_KeyPress(KeyAscii As Integer)
    Call TxtYesNo(TxtChq_AcPayee, KeyAscii)
End Sub

Sub TxtYesNo(txt As TextBox, KeyAscii As Integer)
    If Chr(KeyAscii) = "Y" Or Chr(KeyAscii) = "y" Then
        txt = "Yes"
        KeyAscii = 0
    ElseIf Chr(KeyAscii) = "N" Or Chr(KeyAscii) = "n" Then
        txt = "No"
        KeyAscii = 0
...
Private Sub TXTClrDate_Validate(Index As Integer, Cancel As Boolean)
    If mMoveFlag = True Then Exit Sub
    If ADDFLAG = 3 Then Exit Sub
    TXTClrDate(0) = RetDate(TXTClrDate(0))
End Sub
...
Private Sub TXTChDate_Validate(Index As Integer, Cancel As Boolean)
    If mMoveFlag = True Then Exit Sub
    If ADDFLAG = 3 Then Exit Sub
    TXTChDate(Index) = RetDate(TXTChDate(Index))
End Sub
...
If TopCtrl1.TopText2 = "Add" Then
    If IsDmsVoucherType(TxtVtYpe(0).Tag, Trim(LblVPrefix)) Then
        MsgBox "This Voucher Type is Imported From Siebel Can't be Added"
        Cancel = True
    End If
End If
Set MyRs = Nothing
        ...
        Case 0
            ...
            Select Case FGridRef.Col
                ...
                Case FAgRefNo
                    ...
                    ElseIf PubBackEnd = "S" Then
                        If Val(FGrid1.TextMatrix(Val(FrameRef.Tag) + ScrolIndex, 5)) > 0 Then
                            Set RstRefHelp = G_FaCn.Execute("SELECT AgRefNo AS RefNo,MIN(V_dATE) as VDate," & cTrim(CStr("ABS(ISNULL(SUM(Cr),0)-ISNULL(SUM(Dr),0))")) & " +' '+ Case When ISNULL(SUM(Cr),0)-ISNULL(SUM(Dr),0)>0 Then 'Cr' When ISNULL(SUM(Cr),0)-ISNULL(SUM(Dr),0)<0 Then 'Dr' End  AS Balance FROM ledgerRef WHERE AGREFTYPE IN ('Advance','New Ref','Ag.Ref.') AND SUBCODE='" & FGrid1.TextMatrix(Val(FrameRef.Tag) + ScrolIndex, 2) & "' GROUP BY SubCode,AgRefNo HAVING ISNULL(SUM(Cr),0)-ISNULL(SUM(Dr),0)>0 ORDER BY MIN(V_dATE),AGREFNO")
                        Else
                            Set RstRefHelp = G_FaCn.Execute("SELECT AgRefNo AS RefNo,MIN(V_dATE) as VDate," & cTrim(CStr("ABS(ISNULL(SUM(Cr),0)-ISNULL(SUM(Dr),0))")) & " +' '+ Case When ISNULL(SUM(Cr),0)-ISNULL(SUM(Dr),0)>0 Then 'Cr' When ISNULL(SUM(Cr),0)-ISNULL(SUM(Dr),0)<0 Then 'Dr' End  AS Balance FROM ledgerRef WHERE AGREFTYPE IN ('Advance','New Ref','Ag.Ref.') AND SUBCODE='" & FGrid1.TextMatrix(Val(FrameRef.Tag) + ScrolIndex, 2) & "' GROUP BY SubCode,AgRefNo HAVING ISNULL(SUM(Cr),0)-ISNULL(SUM(Dr),0)<0 ORDER BY MIN(V_dATE),AGREFNO")
                        End If
                    End If
...
Select Case Index
    Case 0
        Select Case FGridRef.Col
            ...
            Case FDueDate
                FGridRef.TextMatrix(FGridRef.Row, FDueDate) = RetDate(TxtGrid(Index))
        End Select
End Select
CalAmountLF
TxtGridLeave = True
...
End If
If UCase(left(PubComp_Name, 4)) = "ENAR" Then TxtSite.Enabled = False
    TxtChq_Favour.Locked = mLock
    TxtChq_AcPayee.Locked = mLock
    FrameChqDetail.Visible = False
    CmdChqPrint.Visible = mLock
...
Else
    ...
    ElseIf PubBackEnd = "S" Then
        mCondStr = " AND Date_From<=" & FaConvertDate(mDate) & " AND ISNULL(DATE_TO," & FaConvertDate(mDate) & " )>=" & FaConvertDate(mDate) & " And (Site_Code = '" & PubSiteCode & "' Or IsNull(Site_Code,'')='') "
    End If
End If
...
'Private Sub PrintVoucherPlain(ByVal VNo1, VType)
...
Dim RST1 As ADODB.Recordset
    ...
    Set Rst = GCn.Execute("Select IsNull(Max(L.SubCode),'') As BankSubCode " & _
                          "From Ledger L " & _
                          "Left Join SubGroup S On L.Subcode=S.SubCode " & _
                          "where S.Nature = 'Bank' And L.DocId = '" & RstMain!DocID & "'")
    If Rst.RecordCount > 0 Then
        If MsgBox("Do You Want to Print Multicity Cheque?", vbYesNo) = vbYes Then
            Set Rst = GCn.Execute("Select IsNull(ChequeReportName,'') as ChequeReportName " & _
                                  "From SubGroup " & _
                                  "Where SubCode = '" & Rst!BankSubCode & "' And ChequeReportName <>''")
        Else
            Set Rst = GCn.Execute("Select IsNull(MulticityChequeReportName,'') as ChequeReportName " & _
                                  "From SubGroup " & _
                                  "Where SubCode = '" & Rst!BankSubCode & "' And ChequeReportName <>''")
        End If
        If Rst.RecordCount > 0 Then
            mRepName = Rst!ChequeReportName
        Else
            MsgBox "Cheque Report Name For Selected Bank Is Not Defined in Ledger A/c Entry"
            Exit Sub
        End If
    End If
