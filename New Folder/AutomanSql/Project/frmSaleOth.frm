...
Begin VB.Form frmSaleOth 
   BackColor       =   &H00BAD3C9&
   Caption         =   "Other Sale Entry"
   ClientHeight    =   8010
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   11595
...
Private Const CrAcCode As Byte = 8
...
'    Set DGDrAc.DataSource = RsDrAc
    ...
    rsForm.Open "Select Name,SubCode from SubGroup", GCn, adOpenDynamic, adLockOptimistic
...
'    RsParty.Open "select SubGroup.SubCode as code,SubGroup.NAME,Party_Type from SubGroup  Where firmCode = '" & PubFirmCode & "' and Nature='Supplier'  order by SubGroup.name", GCn, adOpenDynamic, adLockOptimistic
    ...
    Call MoveRec
   Exit Sub
ELoop:    MsgBox err.Description & vbCr & "In Form_Load Of " & Me.Name
End Sub
...
On Error GoTo errlbl
    ...
    If IsValid(txt(CrAcCode), "Form Type") = False Then Exit Sub
    ...
    If TopCtrl1.TopText2.CAPTION <> "Add" Then   'Edit Case
        GCn.Execute ("update sp_purch  set Cash_Credit = '" & txt(VType) & "', Party_Code == '" & txt(Party).Tag & "', Party_Name= '" & txt(Party) & _
            "', Party_Doc_No ='" & txt(SuppChlNo) & "',Party_Doc_Date =" & ConvertDate(txt(SuppChlDate)) & ",L_C = '" & left(txt(LC), 1) & "',form_code = '" & rsForm!SubCode & _
            "', remarks  = '" & txt(Remark) & "', Tot_Goods_Value=" & Val(txt(NetAmt)) & ",NET_AMT = " & Val(txt(NetAmt)) & _
            ",U_Name='" & pubUName & "', U_EntDt=#" & PubServerDate & "#, U_AE='E', DrAc_Code='" & txt(CrAcCode).Tag & _
            "' where DocId = '" & txt(TxtDocID) & "'")
    Else    'Add
        SQLPBill = "insert into sp_purch(DocID,DocIDHelp,V_Type,V_No,Site_Code," _
            ...
            & "Party_Doc_Date,L_C,Tot_Goods_Value," _
            ...
            "'," & ConvertDate(txt(SuppChlDate)) & ",'" & left(txt(LC), 1) & "', " & Val(txt(NetAmt)) & _
            ", " & Val(txt(NetAmt)) & ",'" & txt(Remark) & "','" & pubUName & "',#" & PubServerDate & "#,'A','" & rsForm!SubCode & "')"
        'Purchase Bill Add
...
Select Case Index
    ...
    Case Party
        ...
        End If
    Case CrAcCode
        DGridTxtKeyDown DGForm, txt, CrAcCode, rsForm, KeyCode, False, 1, frmTaxForms, "frmTaxForms"
'    Case PermitType
...
Select Case Index
    ...
    Case Party
        ...
        If txt(VType).TEXT = "Cash" Then
            ...
            GSQL = "Select OrderID as Code,Order_Reg_No as Name,Order_Reg_Dt, Trim(" & cMID("OrderID", "8", "5") & ")+CStr(Trim(Right(OrderID,8))) as OurDocNo,V_Date From SP_Order Where left(OrderID,1)='" & PubDivCode & "' and left(Order_Type,4)='S_PO' and V_Date<=#" & Format(txt(VDate), "dd-mmm-yyyy") & "# and OrdClosDate is null Order By OrderID"
...
'            txt(CrAcCode).Tag = IIf(IsNull(rsForm!PurSal_Ac_Code), "", rsForm!PurSal_Ac_Code)
'            txt(CrAcCode) = GCn.Execute("Select SubCode,Name from subgroup where SubCode='" & txt(CrAcCode).Tag & "'").Fields(0).Value
   End If
    Case SuppChlDate
...
Private Sub DGForm_Click()
    If rsForm.RecordCount > 0 Then
        txt(CrAcCode).TEXT = rsForm!Name
        txt(CrAcCode).Tag = rsForm!SubCode
    End If
    txt(CrAcCode).SetFocus
...
If Master.RecordCount > 0 Then
    ...
    End If
    'txt(CrAcCode).Tag = IIf(IsNull(Master1!DrAc_Code), "", Master1!DrAc_Code)
    'txt(CrAcCode) = GCn.Execute("Select Name from subgroup where SubCode='" & Master1!DrAc_Code & "'").Fields(0).Value
    ...
    'txt(CrAcCode).Tag = IIf(IsNull(Master1!form_Code), "", Master1!form_Code)
    'If txt(CrAcCode).Tag <> "" Then
     '   txt(CrAcCode) = GCn.Execute("select form_desc from taxforms where form_code = '" & txt(CrAcCode).Tag & "'").Fields(0).Value
    'Else
     '   txt(CrAcCode) = ""
    'End If
