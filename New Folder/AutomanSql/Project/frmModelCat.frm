...
Private Const ModelCat_Code = 0, ModelCat_NAME = 1
...
If PubMoveRecYn Then
    RstMain.Open "Select ModelCat_Code as searchcode,MODEL_CAT.* From MODEL_CAT  where left(ModelCat_Code,1)='" & PubDivCode & "' " & sitecond & " Order by ModelCat_Name", GCn, adOpenDynamic, adLockOptimistic
Else
    RstMain.Open "Select Top 1 ModelCat_Code as searchcode,MODEL_CAT.* From MODEL_CAT  where left(ModelCat_Code,1)='" & PubDivCode & "' " & sitecond & " Order by ModelCat_Name", GCn, adOpenDynamic, adLockOptimistic
End If
...
On Error GoTo ErrLoop
    ...
    If TopCtrl1.TopText2 = "Add" Then
        GCn.Execute ("DELETE From MODEL_CAT Where ModelCat_Code=" & Chk_Text(Trim(Txt(ModelCat_Code))))
        GCn.Execute ("Insert Into MODEL_CAT (ModelCat_Code,Site_Code,ModelCat_Name,U_Name,U_EntDt,U_AE) Values('" & Trim(Txt(ModelCat_Code)) & "','" & PubSiteCode & "'," & Chk_Text(Txt(ModelCat_NAME)) & ",'" & pubUName & "'," & ConvertDate(PubServerDate) & ",'" & IIf(ADDFLAG = 1, "A", "E") & "')")
    Else
        GCn.Execute ("update  MODEL_CAT set Site_Code='" & PubSiteCode & "',ModelCat_Name=" & Chk_Text(Txt(ModelCat_NAME)) & ",U_Name='" & pubUName & "',U_EntDt=" & ConvertDate(PubServerDate) & ",U_AE='" & IIf(ADDFLAG = 1, "A", "E") & "'" & " Where ModelCat_Code=" & Chk_Text(Trim(Txt(ModelCat_Code))))
    End If
    GCn.CommitTrans
