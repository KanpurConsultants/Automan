...
Public Sub CreateLog(frm As Form, mDocId As String, mRepostingYn As Boolean)
    On Error GoTo err
    ...
    Dim mBillAmt As Double
    Dim mAddition As Double
    Dim mDeduction As Double

    Select Case frm.Name
        Case "frmVehSale"
            Set RsTemp = GCn.Execute("Select VO.Inv_Date as VDate,'' As VType, Vo.Rebate As Discount, VO.Net_Amount as BillAmt, OtherChrg As Addition, 0 As Deduction, 0 As Lab_D_Amt, 0 As NetLab_Amt From Veh_Order VO Left Join Veh_Stock VS On VO.Inv_DocID=VS.Sal_DocId Where VO.Inv_DocId='" & mDocId & "'")
            If RsTemp.RecordCount > 0 Then
                Set rsTemp1 = GCn.Execute("Select Count(*) As Item, Count(*) As Qty, Sum(Sal_Rate) As Amt From Veh_Stock Where Sal_DocId='" & mDocId & "'")
            End If
        
        Case "frmSaleBill"
            Set RsTemp = GCn.Execute("Select V_Date As VDate, Cash_Credit As VType, Total_Amt As BillAmt, Addition, 0 As Deduction, D_Amt_TB+D_Amt_TP As Discount, 0 As Lab_D_Amt, 0 As NetLab_Amt  From Sp_Sale Where DocId='" & mDocId & "'")
            If RsTemp.RecordCount > 0 Then
                Set rsTemp1 = GCn.Execute("Select Count(*) As Item, Sum(Qty_Iss) As Qty, Sum(Net_Amt2) As Amt From Sp_Stock Where Invoice_DocId='" & mDocId & "'")
            End If
        
        Case "frmPurBill"
            Set RsTemp = GCn.Execute("Select V_Date As VDate, Cash_Credit As VType, Addition, Deduction, Net_Amt As BillAmt, Tot_Disc_Amt as Discount, 0 As Lab_D_Amt, 0 As NetLab_Amt  From Sp_Purch Where DocId='" & mDocId & "'")
            If RsTemp.RecordCount > 0 Then
                Set rsTemp1 = GCn.Execute("Select Count(*) As Item, Sum(Qty_Rec) As Qty, Sum(Net_Amt2) As Amt From Sp_Stock Where Invoice_DocId='" & mDocId & "'")
            End If
        
        Case "frmVehPur"
            Set RsTemp = GCn.Execute("Select V_Date as VDate,'' As VType, 0 As Discount, Tot_Amount as BillAmt, Addition,  Deduction, 0 As Lab_D_Amt, 0 As NetLab_Amt  From Veh_Purch1 Where DocId='" & mDocId & "'")
            If RsTemp.RecordCount > 0 Then
                Set rsTemp1 = GCn.Execute("Select Count(*) As Item, Count(*) As Qty, Sum(Rate) As Amt From Veh_Stock Where Pur_DocId='" & mDocId & "'")
            End If
        
        Case "frmJobClose"
            Set RsTemp = GCn.Execute("Select J.JobCloseDate As VDate, S.Cash_Credit As VType, S.Total_Amt As BillAmt, S.Addition, 0 As Deduction, S.D_Amt_TB+S.D_Amt_TP As Discount, J.Lab_D_Amt, J.NetLab_Amt From Job_Card J Left Join Sp_Sale S On J.DocId=S.Job_DocId Where J.DocId='" & mDocId & "'")
            If RsTemp.RecordCount > 0 Then
                Set rsTemp1 = GCn.Execute("Select Count(*) As Item, Sum(Qty_Iss) As Qty, Sum(Net_Amt2) As Amt From Sp_Stock Where Job_DocId='" & mDocId & "'")
            End If
            
        Case "FaVrEnt"
            Set RsTemp = GCn.Execute("Select Max(L.V_Date) As VDate, Max(Vt.Description) As VType, Sum(L.AmtDr) As BillAmt, 0 as Addition, 0 As Deduction, 0 As Discount, 0 as Lab_D_Amt, 0 as NetLab_Amt From Ledger L Left Join Voucher_Type Vt On L.V_Type=Vt.V_Type Where L.DocId='" & mDocId & "' Group By L.DocId")
            If RsTemp.RecordCount > 0 Then
                Set rsTemp1 = GCn.Execute("Select 0 As Item, 0 As Qty, 0 As Amt ")
            End If
    End Select
    ...
    If rsTemp1.RecordCount > 0 Then
        ...
        mDiscount = VNull(RsTemp!Discount)
        mAddition = VNull(RsTemp!Addition)
        mDeduction = VNull(RsTemp!Deduction)
    End If
    
    GCn.Execute "Insert Into DeleteLog(DocId, Type, VType, VDate, Total_Item, " & _
                        "Total_Qty, GoodsValue, Bill_Amt, Discount, Addition, " & _
                        "Deduction, AutoYn, User_Name, Del_Date, Del_Time, EditDate, EditTime, LabDiscount, LabAmount) Values( " & _
                        "'" & mDocId & "', '" & left(frm.CAPTION, 10) & "', '" & XNull(RsTemp!VType) & "', " & ConvertDate(XNull(RsTemp!VDate)) & ", " & mTotalItem & ", " & _
                        "" & mTotalQty & ", " & mGoodsValue & ", " & VNull(RsTemp!BillAmt) & ", " & mDiscount & ", " & mAddition & ", " & _
                        "" & mDeduction & ", '" & IIf(mRepostingYn, "Y", "N") & "', '" & pubUName & "', " & _
                        "" & ConvertDate(IIf(frm.TopCtrl1.TopText2 = "Browse", PubLoginDate, "")) & ", '" & time & "', " & _
                        "" & ConvertDate(IIf(frm.TopCtrl1.TopText2 = "Edit", PubLoginDate, "")) & ", '" & time & "', " & VNull(RsTemp!Lab_D_Amt) & ", " & VNull(RsTemp!NetLab_Amt) & ")"
