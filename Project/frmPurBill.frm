...
Private Const TaxForm As Byte = 16
Private Const TaxPer As Byte = 17
Private Const TaxAmt1 As Byte = 18
Private Const SatPer As Byte = 19
Private Const SatAmt1 As Byte = 20

Private Const ItemVal As Byte = 21
Private Const Godown As Byte = 22
Private Const NDP  As Byte = 23
Private Const PartSrlNo As Byte = 24         ' Part Serial N
...
Private Const God As Byte = 37
Private Const PONOCode As Byte = 38
Private Const POSrlNo As Byte = 39
Private Const ChlDocId As Byte = 40
Private Const ChlDocSr As Byte = 41
Private Const PartGrade As Byte = 42
Private Const EffectDate As Byte = 43
Private Const MRPRate As Byte = 44
Private Const TaxFormCode As Byte = 45
...
Dim I As Byte
    ...
    rsCtrlAc.Open "Select SprPurTrans_Ac,EntryTax_Ac,SprCash_Ac,SprDiscTB_Ac,MiscChrg_Ac From AcControls", GCnFaS, adOpenDynamic, adLockOptimistic
    'eof checking
            ...
            For I = 1 To FGrid.Rows - 1
                If FGrid.TextMatrix(I, PNo) <> "" And Val(FGrid.TextMatrix(I, PQty)) <> 0 Then
                    GCn.Execute ("insert into sp_stock(DocID,Srl_No,V_Type,V_No,V_Date,Party_Code,L_C,Order_DocId, " & _
                        ...
                        " v_date2,Invoice_DocId,Rate2, Amount2, Disc_Per2, Disc_Amt2, Ord_DiscPer2, Ord_DiscAmt2, Net_Amt2, TaxPer, TaxAmt, SatPer, SatAmt , SFCPer, SFCAmt,TaxForm ) " & _
...
If Master.RecordCount > 0 Then
    ...
    Set Rs = GCn.Execute("SELECT SPO.Order_Reg_No,P.Part_Name, " & cTrim(cMID("Stk.Order_DocID", "8", "5")) & " + " & cCStr(cTrim("Right(Stk.Order_DocID,8)")) & " As OrderIDDisp, " & _
            ...
            " P.Cur_TB_Stk, P.Cur_TP_Stk, P.TP_SRate, P.TB_SRate, P.Bin_Loca, P.High_Pur_Rate, P.Low_Pur_Rate, Stk.*, G.God_Name, TaxForms.Form_Desc " & _
            ...
            " Left Join TaxForms on Stk.TaxForm=TaxForms.Form_Code " & _
...
'                .TextMatrix(i, PONo) = IIf(IsNull(rs!OrderIDDisp), "", rs!OrderIDDisp)
                ...
                .TextMatrix(I, TaxForm) = XNull(Rs!form_desc)
                .TextMatrix(I, TaxFormCode) = XNull(Rs!TaxForm)
...
'    FGrid.FormatString = "SrNo.|Part No.            |Part Name             |Unit |Godown          |PO No.         |Tax Y/N|MRP Y/N| Qty(Doc)|Qty(Phy)|Rate     |Amount    |Dis %    |Dis Rs   |Ord Dis %  |Ord Dis Rs  |NDP     |ItemValue   |Local Name|Curr Stk Qty|MRP Qty|Taxable Qty|TaxPaid Qty|Taxable Rate|TaxPaid Rate|Bin Location|Last Purch Rate|High Purch Rate|Low Purch Rate"
    ...
    With FGrid
        ...
        .TextMatrix(0, TaxForm) = "Tax Form"
        .ColAlignment(TaxForm) = flexAlignLeftCenter
        .ColWidth(TaxForm) = 650
        ...
        Set rsTaxPer = GCn.Execute("Select Tax_Per, AddTaxPer,L_C,SFCPER  from TaxForms where Form_Code='" & Txt(FormType).Tag & "'")
         If rsTaxPer.RecordCount > 0 Then
            ...
            If UTrim(XNull(rsTaxPer!L_C)) = "LOCAL" Then
               ...
               Set rsTaxPer = GCn.Execute("Select TaxForms.Form_Desc, TaxFormPurchase, TaxForms.Tax_Per VatPer, TaxForms.AddTaxPer From Part_Grade Left Join TaxForms On Part_Grade.TaxFormPurchase = TaxForms.Form_Code Where PartGrade_Code='" & FGrid.TextMatrix(FGrid.Row, PartGrade) & "'")
               If rsTaxPer.RecordCount > 0 Then
                    FGrid.TextMatrix(FGrid.Row, TaxFormCode) = XNull(rsTaxPer!TaxFormPurchase)
                    FGrid.TextMatrix(FGrid.Row, TaxForm) = XNull(rsTaxPer!form_desc)
...
If CDate(Txt(VDate)) > CDate("31/Mar/14") Then
    mQry = "SELECT Max(IsNull(Tf.PurSal_Ac_Code, DTF.PurSal_Ac_Code )) AS PurSal_Ac_Code, " & _
           ...
           "LEFT JOIN TaxFormsAc DTF ON DTF.Form_Code ='" & Txt(FormType).Tag & "' " & _
           "WHERE Invoice_DocId ='" & DocID & "' " & _
           "GROUP BY IsNull(TF.Form_Code, DTF.Form_Code) "
    Set RsStk = GCn.Execute(mQry)
    
    mQry = "select Addition, Deduction " & _
        "from SP_Purch " & _
        "where docid='" & DocID & "'"
...
'    Set RsLab = GCn.Execute(mQry)
    ...
    If Txt(VType).TEXT = "Cash" Then
        mNarr = "Cash purchase bill no. " & Txt(SuppChlNo) & " dated : " & Txt(SuppChlDate)
    Else
        mNarr = "Credit purchase  bill no. " & Txt(SuppChlNo) & " dated : " & Txt(SuppChlDate)
    End If
    ...
    If RsStk.RecordCount > 0 Then
        Do While Not RsStk.EOF
            If VNull(RsStk!Taxable_Amt) > 0 Then
                ...
                LedgAry(UBound(LedgAry)).AmtDr = VNull(RsStk!Taxable_Amt)
            ...
            If VNull(RsStk!TaxAmt) > 0 Then
                ...
                LedgAry(UBound(LedgAry)).AmtDr = VNull(RsStk!TaxAmt)
                LedgAry(UBound(LedgAry)).AmtCr = 0
                LedgAry(UBound(LedgAry)).Narration = mNarr
            End If
            ...
            If VNull(RsStk!AddTaxAmt) > 0 Then
                ...
                LedgAry(UBound(LedgAry)).AmtDr = VNull(RsStk!AddTaxAmt)
                LedgAry(UBound(LedgAry)).AmtCr = 0
                LedgAry(UBound(LedgAry)).Narration = mNarr
            End If
    ...
    If RsSale.RecordCount > 0 Then
            ...
            If VNull(RsSale!Deduction) > 0 Then
                ReDim Preserve LedgAry(UBound(LedgAry) + 1)
                LedgAry(UBound(LedgAry)).SubCode = XNull(rsCtrlAc!SprDiscTB_Ac)
                LedgAry(UBound(LedgAry)).AmtDr = 0
                LedgAry(UBound(LedgAry)).AmtCr = VNull(RsSale!Deduction)
                LedgAry(UBound(LedgAry)).Narration = mNarr
            End If
            
            
            If VNull(RsSale!Addition) > 0 Then
                ReDim Preserve LedgAry(UBound(LedgAry) + 1)
                LedgAry(UBound(LedgAry)).SubCode = XNull(rsCtrlAc!MiscChrg_Ac)
                LedgAry(UBound(LedgAry)).AmtDr = VNull(RsSale!Addition)
                LedgAry(UBound(LedgAry)).AmtCr = 0
                LedgAry(UBound(LedgAry)).Narration = mNarr
            End If
    ...
    If Val(Txt(NetAmt)) > 0 Then
        ...
        LedgAry(UBound(LedgAry)).AmtDr = 0
        LedgAry(UBound(LedgAry)).AmtCr = Val(Txt(NetAmt))
        LedgAry(UBound(LedgAry)).Narration = mNarr
    End If
    
    
    
    mResult = LedgerPost(left(TopCtrl1.TopText2, 1), LedgAry, GCnFaS, DocID, CDate(Txt(VDate)), mCommNarr)
