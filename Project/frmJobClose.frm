...
Private Const Col_TaxForm As Byte = 15
Private Const Col_TaxPer As Byte = 16
Private Const Col_TaxAmt As Byte = 17
Private Const Col_SatPer As Byte = 18
Private Const Col_SatAmt As Byte = 19

Private Const Col_ItemVal As Byte = 20
Private Const Col_Purpose As Byte = 21
Private Const Col_PName As Byte = 22
Private Const Col_LName As Byte = 23
Private Const Col_ClaimNo As Byte = 24
Private Const Col_CompYN As Byte = 25
Private Const Col_PartGrade As Byte = 26
...
If CDate(Txt(JobCDt)) > CDate("31/Mar/14") Then
    mQry = "SELECT Max(IsNull(Tf.PurSal_Ac_Code, DTF.PurSal_Ac_Code )) AS PurSal_Ac_Code, " & _
           "Max(IsNull(Tf.Tax_Ac_Code, DTF.Tax_Ac_Code )) AS Tax_Ac_Code, " & _
           "Max(IsNull(Tf.Sur_Ac_Code, DTF.Sur_Ac_Code )) AS Sur_Ac_Code, " & _
           "Round(Sum(Stk.Net_Amt2),2) AS Taxable_Amt, Round(Sum(Stk.TaxAmt),2) AS TaxAmt, Round(Sum(Stk.SatAmt),2)   AS AddTaxAmt " & _
           "FROM SP_Stock Stk " & _
           "LEFT JOIN TaxFormsAc TF ON Tf.Form_Code =Stk.TaxForm " & _
           "LEFT JOIN TaxFormsAc DTF ON DTF.Form_Code ='" & mFormCode & "' " & _
           "WHERE Invoice_DocId ='" & SpareDocID & "' " & _
           "GROUP BY IsNull(TF.Form_Code, DTF.Form_Code) "
    Set RsStk = GCn.Execute(mQry)
    
    mQry = "select sum(rounded) as RoundAmt, sum(D_Amt_TB) as DisAmtTB, " & _
        "sum(Gen_Sur_Amt) as GenSurAmt,sum(Trans_Amt) as Trans," & _
        "sum(Packing) as Pack,  sum(ReSalTax_Amt) as ReSaleTaxAmt " & _
        "from SP_Sale " & _
        "where docid='" & SpareDocID & "'"
    Set RsSale = GCn.Execute(mQry)
    
    mQry = "Select sum(LabAmt_TB)-IsNull(sum(Lab_D_Amt),0) as LabAmt_TB,sum(LabAmt_TP) as LabAmt_TP,sum(Lab_D_Amt) as Lab_D_Amt" & _
        ",sum(Lab_TaxAmt) as Lab_TaxAmt,sum(Lab_RoundOff) as Lab_RoundOff,sum(NetLab_Amt) as NetLab_Amt " & _
        "from Job_Card where JobCloseDate=" & ConvertDate(Txt(JobCDt)) & _
        " and DocId='" & Txt(JobNo).Tag & "'"
    Set RsLab = GCn.Execute(mQry)
    
    
    If Txt(CashBill) = "Yes" Then
        mNarr = "Workshop cash bill for job card no : " & Txt(JobNo) & " vehicle no : " & Txt(VehRegNo)
    Else
        mNarr = "Workshop credit bill for job card no : " & Txt(JobNo) & " vehicle no : " & Txt(VehRegNo)
    End If
    ...
    If RsStk.RecordCount > 0 Then
        Do While Not RsStk.EOF
            If VNull(RsStk!Taxable_Amt) > 0 Then
                ReDim Preserve LedgAry(UBound(LedgAry) + 1)
                LedgAry(UBound(LedgAry)).SubCode = XNull(RsStk!PurSal_Ac_Code)
                LedgAry(UBound(LedgAry)).AmtDr = 0
                LedgAry(UBound(LedgAry)).AmtCr = VNull(RsStk!Taxable_Amt)
                LedgAry(UBound(LedgAry)).Narration = mNarr
            End If
            
            If VNull(RsStk!TaxAmt) > 0 Then
                ReDim Preserve LedgAry(UBound(LedgAry) + 1)
                LedgAry(UBound(LedgAry)).SubCode = XNull(RsStk!Tax_Ac_Code)
                LedgAry(UBound(LedgAry)).AmtDr = 0
                LedgAry(UBound(LedgAry)).AmtCr = VNull(RsStk!TaxAmt)
                LedgAry(UBound(LedgAry)).Narration = mNarr
            End If
            
            If VNull(RsStk!AddTaxAmt) > 0 Then
                ReDim Preserve LedgAry(UBound(LedgAry) + 1)
                LedgAry(UBound(LedgAry)).SubCode = XNull(RsStk!Sur_Ac_Code)
                LedgAry(UBound(LedgAry)).AmtDr = 0
                LedgAry(UBound(LedgAry)).AmtCr = VNull(RsStk!AddTaxAmt)
                LedgAry(UBound(LedgAry)).Narration = mNarr
            End If
    ...
    If RsSale.RecordCount > 0 Then
            ...
            If VNull(RsSale!DisAmtTB) > 0 Then
                ReDim Preserve LedgAry(UBound(LedgAry) + 1)
                LedgAry(UBound(LedgAry)).SubCode = XNull(rsCtrlAc!SprDiscTB_Ac)
                LedgAry(UBound(LedgAry)).AmtDr = VNull(RsSale!DisAmtTB)
                LedgAry(UBound(LedgAry)).AmtCr = 0
                LedgAry(UBound(LedgAry)).Narration = mNarr
            End If
            
            If VNull(RsSale!GenSurAmt) > 0 Then
                ReDim Preserve LedgAry(UBound(LedgAry) + 1)
                LedgAry(UBound(LedgAry)).SubCode = XNull(rsCtrlAc!SprGenSur_Ac)
                LedgAry(UBound(LedgAry)).AmtDr = 0
                LedgAry(UBound(LedgAry)).AmtCr = VNull(RsSale!GenSurAmt)
                LedgAry(UBound(LedgAry)).Narration = mNarr
            End If
            
            If VNull(RsSale!Trans) > 0 Then
                ReDim Preserve LedgAry(UBound(LedgAry) + 1)
                LedgAry(UBound(LedgAry)).SubCode = XNull(rsCtrlAc!Transportation_Ac)
                LedgAry(UBound(LedgAry)).AmtDr = 0
                LedgAry(UBound(LedgAry)).AmtCr = VNull(RsSale!Trans)
                LedgAry(UBound(LedgAry)).Narration = mNarr
            End If
            
            If VNull(RsSale!Pack) > 0 Then
                ReDim Preserve LedgAry(UBound(LedgAry) + 1)
                LedgAry(UBound(LedgAry)).SubCode = XNull(rsCtrlAc!MiscChrg_Ac)
                LedgAry(UBound(LedgAry)).AmtDr = 0
                LedgAry(UBound(LedgAry)).AmtCr = VNull(RsSale!Pack)
                LedgAry(UBound(LedgAry)).Narration = mNarr
            End If
            
            If VNull(RsSale!RoundAmt) + VNull(RsLab!Lab_RoundOff) <> 0 Then
                ReDim Preserve LedgAry(UBound(LedgAry) + 1)
                LedgAry(UBound(LedgAry)).SubCode = XNull(rsCtrlAc!SprROff_Ac)
                LedgAry(UBound(LedgAry)).AmtDr = IIf(VNull(RsSale!RoundAmt) + VNull(RsLab!Lab_RoundOff) < 0, Abs(VNull(RsSale!RoundAmt) + VNull(RsLab!Lab_RoundOff)), 0)
                LedgAry(UBound(LedgAry)).AmtCr = IIf(VNull(RsSale!RoundAmt) + VNull(RsLab!Lab_RoundOff) > 0, Abs(VNull(RsSale!RoundAmt) + VNull(RsLab!Lab_RoundOff)), 0)
                LedgAry(UBound(LedgAry)).Narration = mNarr
            End If
    End If
    
    If RsLab.RecordCount > 0 Then
        If VNull(RsLab!LabAmt_TB) <> 0 Then
            ReDim Preserve LedgAry(UBound(LedgAry) + 1)
            LedgAry(UBound(LedgAry)).SubCode = rsCtrlAcLab!SrvLabourTB_Ac    'Taxable Labour A/c Code
            LedgAry(UBound(LedgAry)).AmtCr = VNull(RsLab!LabAmt_TB)
            LedgAry(UBound(LedgAry)).Narration = mNarr & "Labour charges"
        End If
        
        If VNull(RsLab!LabAmt_TP) <> 0 Then
            ReDim Preserve LedgAry(UBound(LedgAry) + 1)
            LedgAry(UBound(LedgAry)).SubCode = rsCtrlAcLab!SrvLabour_Ac
            LedgAry(UBound(LedgAry)).AmtCr = VNull(RsLab!LabAmt_TP)
            LedgAry(UBound(LedgAry)).Narration = mNarr & "Labour charges"
        End If
        
        
        If VNull(RsLab!Lab_TaxAmt) <> 0 Then
            ReDim Preserve LedgAry(UBound(LedgAry) + 1)
            LedgAry(UBound(LedgAry)).SubCode = rsCtrlAcLab!SrvTax_Ac
            LedgAry(UBound(LedgAry)).AmtCr = VNull(RsLab!Lab_TaxAmt)
            LedgAry(UBound(LedgAry)).Narration = mNarr & " service tax on labour charges"
        End If
    End If
    ...
    If Val(Txt(NetSprAmt)) + Val(Txt(NetLabAmt)) > 0 Then
        ReDim Preserve LedgAry(UBound(LedgAry) + 1)
        LedgAry(UBound(LedgAry)).SubCode = PartyCode
        LedgAry(UBound(LedgAry)).AmtDr = Val(Txt(NetSprAmt)) + Val(Txt(NetLabAmt))
        LedgAry(UBound(LedgAry)).AmtCr = 0
        LedgAry(UBound(LedgAry)).Narration = mNarr
    End If
...
Private Sub FPostFreeServiceCouponValue()
    Dim I As Integer
    Dim xServTaxAmt As Double
    Dim SrvCat As String
    Dim RsTemp As ADODB.Recordset
    Dim mDName$, mDCity$, mDCode$, dSubCode$
    Dim mGroupCode$, TmpSQL$, mNarr$
    Dim FSBVal As Double
    Dim mServTaxPer As Double
    Dim LedgAry() As LedgRec, LedgAryLab() As LedgRec, mCommNarr$, mLabSQL$
...
MeWidth = Me.width
    ...
    With FGrid
        ...
        .TextMatrix(0, Col_TaxForm) = "Tax Form"
        .ColAlignmentFixed(Col_TaxForm) = flexAlignRightCenter
        .ColWidth(Col_TaxForm) = 840
    ...
    If Rst.RecordCount > 0 Then
        Do Until Rst.EOF
            ...
            With FGrid
                ...
                .TextMatrix(I, Col_TaxForm) = XNull(Rst!Form_Desc)
    ...
    Else
        ...
        If mSepLabInv = 0 Then  'No, Merge Invoice=Spare + Labour
            GSQL = "SELECT '1' as Orig,JC.AtKMsHrs,JC.Lab_D_Amt,JC.ArrivalTime,JC.JobComp_Dt_Time,SPStk.DocID as ReqDocID, " & vIsNull("SPStk.Srl_No", "0") & " as Srl_No,JC.DocID as JobDocID,JC.CrMemo,H.Model,H.RegNo,H.Chassis,JC.DocId_InvSpr,JC.DocID_InvLab,JC.JobCloseDate as v_Date,s.Party_Code,s.Party_Name,s.Address,SG.NamePrefix,H.Name,H.Add1,H.Add2,H.Add3,City.CityName,SG.PIN,SG.Phone," & _
                ...
                "Syctrl.WorkShopInvFooter," & vIsNull("Syctrl.SrvGatePass_On", "0") & " as SrvGatePass_On," & vIsNull("Syctrl.SrvGatePass", "0") & " as SrvGatePass,SPStk.TaxPer," & cIIF("SPStk.Purpose='C' Or SPStk.Purpose='W'", "SPStk.TaxAmt", "0") & " As TaxAmt,Jc.Job_No, " & cCStr(xIsNull("SpStk.v_No", "")) & " as ReqNo,Jc.LastInvNoSuff,JC.LastLabInvNoSuff, JC.Job_Date, H.PhoneOff, H.PhoneResi, H.Mobile, " & xIsNull("P.Unit", "Each") & " As Unit, 0 as eCessPer, 0 as eCessAmt, (Select SrvTaxNo From Syctrl) As SrvTaxNo, SG.LstNo As LstNoParty, H.PhoneOff, H.PhoneResi, H.Mobile, " & vIsNull("MG.ModelGrp_Name", "M.Model") & "  As ModelGrp_Name, SG.SiebelCode, SpStk.SatPer, SpStk.SatAmt, " & Val(Txt(SatAmt)) & " as SatAmt_H,P.Unit,Syctrl.HelpLineNo, 0 as ServiceTaxPer_Saperate, 0 as ServiceTaxAmt_Saperate  " & _
...
'Print Area 18
    ...
    Print #1, mSP2 & ""
    mHeader = mHeader + 1
    Print #1, Space((PageWidth - Len("")) / 2) & mChr17 & "" & mChr18
    Print #1, mEject
    Close #1
    Open "C:\RepPrint.Bat" For Output As #1
'    If fob.FolderExists("c:\WinNt") Then
...
'                    ; " | " & PSTR("V A T     ", 10, 0) & Space(6) & PSTR(RstJob!Tax_Amt, 12, 2) & mDoub
    ...
    For I = 1 To Len(Footer)
       ...
       End If
    Next
    Print #1, mSP2 & Space((((PageWidth) * 1.7) - Len("" & "   " & pubUName & "   " & PubServerDate)) / 2) & "" & "   " & pubUName & "   " & PubServerDate & mChr18
'Gate Pass Footer()
    'If RstJob!Printed_YN = 0 Then
    If Not Provisional Then
        If RstJob!SrvGatePass = 1 And RstJob!SrvGatePass_On = "S" Then
            ...
            Print #1, mSP2 & mChr17 & Space((((PageWidth) * 1.7) - Len("" & "   " & pubUName & "   " & PubServerDate)) / 2) & "" & "   " & pubUName & "   " & PubServerDate & mChr18
        End If
    End If
    'End If
    'End If
...
' SALE FOOTER
    ...
    PrintStr = PrintStr & " |Serv.Tax @ " & PSTR(14#, 5, 2) & ":" & PSTR(Val(Txt(ServTaxAmt)) - Val(Txt(eCessAmt)), 9, 2) & " |" & mEmph & "Net Labour Rs.    : " & PSTR(Val(Txt(NetLabAmt)), 9, 2) & mEmph1
    Print #1, mSP2 & PrintStr
    PrintStr = mLabDiscAmtStr
    PrintStr = PrintStr & " |" & "Round Off       :  " & PSTR(Val(Txt(LabROff)), 7, 2) & " |" & mEmph & "Net Payble Amt Rs.: " & PSTR(Val(Txt(NetAmt)), 9, 2) & mEmph1
    Print #1, mSP2 & PrintStr
    PrintStr = Space(19) & " |" & "Swachh Bharat Cess @ 0.5%    :  " & PSTR(Val(Txt(eCessAmt)), 7, 2)
    Print #1, mSP2 & PrintStr
    Print #1, mSP2 & Replace(Space(PageWidth), " ", "-")
    Print #1, mSP2 & mDoub & ntow(Txt(NetAmt), "Rupees", "Paise") & mDoub1
    Print #1, mSP2 & Replace(Space(PageWidth), " ", "-")
