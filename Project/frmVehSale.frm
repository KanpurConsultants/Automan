...
'    "left join city on left(ContractFinance.City,4)=City.CityCode where fincatg = 0  order by finname", GCn, adOpenDynamic, adLockOptimistic
    Set rsFin = GCn.Execute("select fincode as code,finname + ',' +  IsNull(add1,'') +  ',' +  IsNull(add2,'') + IsNull(City.CityName,'') as name, Add1, Add2,AcCode,FinBankCode, UnderFinGrp, FinName from ContractFinance left join city on left(ContractFinance.City,4)=City.CityCode where fincatg = 0  order by finname")
...
'        Print #1, mSP5 & mChr17 & RstCert!Inv_UName & Space(((PageWidth * 1.7) - Len("") - Len(RstCert!Inv_UName)) / 2) & "" & mChr18
    Print #1, mEject
    Close #1
...
'        Print #1, PRN_TIT(Trim(RstCert!Manufacturer), "B", PageWidth)
        ...
        Loop
        Print #1, mSP5 & Replace(Space(PageWidth), " ", "-")
'        Print #1, mChr17 & RstCert!Inv_UName & Space(((PageWidth * 1.7) - Len("") - Len(RstCert!Inv_UName)) / 2) & "" & mChr18
    Print #1, mEject
    Close #1
    Open "C:\RepPrint.Bat" For Output As #1
'    If fob.FolderExists("c:\WinNt") Then
...
'Print Area 18
 ...
 '0 -Hypothecation ,1- Hire purchase ,2 -Own Fund,3- Lease, 4-Agreement, 5-Lease & Agreement
    ...
    ElseIf Rstsale!Fund_Source = 1 Or _
        ...
        Right2 = Txt(NamePrefix) & " " & XNull(Rstsale!Name)
   ...
   If UCase(left(PubComp_Name, 4)) = "COMM" Then
    ...
    mHeader = mHeader + 1
    Print #1, PSTR(IIf(Trim(Left2) = "", "--", Left2), 40) & Space(10) & PSTR(Right2, 40)
    mHeader = mHeader + 1
    Print #1, PSTR(IIf(Trim(Left3) = "", "--", Left3), 40) & Space(10) & PSTR(Right3, 40)
    mHeader = mHeader + 1
    Print #1, PSTR(IIf(Trim(Left4) = "", "--", Left4), 40) & Space(10) & PSTR(Right4, 40)
    mHeader = mHeader + 1
    Print #1, PSTR(IIf(Trim(Left5) = "", "--", Left5), 40) & Space(10) & PSTR(Right5, 40)
    mHeader = mHeader + 1
    Print #1, PSTR(IIf(Trim(Left6) = "", "--", Left6), 40) & Space(10) & PSTR(Right6, 40)
    mHeader = mHeader + 1
    ...
    If Rstsale!Fund_Source = 0 Or Rstsale!Fund_Source = 2 Or Rstsale!Fund_Source = 3 Or Rstsale!Fund_Source = 6 Then
        ...
        mHeader = mHeader + 1
    Else
        Print #1, Space(50) & PSTR("Phone : " & XNull(Rstsale!Phone) & "," & XNull(Rstsale!Mobile), 40)
        mHeader = mHeader + 1
    End If
    ...
    'mHeader = mHeader + 1
    If RstInvDet!SupInvOnVehSaleInv = 1 Then
        ...
        mHeader = mHeader + 1
    Else
        Print #1, IIf(XNull(Rstsale!PanNo) <> "", PSTR("PAN No.: " & XNull(Rstsale!PanNo), 50), Space(50)) & mEmph & "Invoice Date : " & STR(Rstsale!Inv_Date) & mEmph1
        mHeader = mHeader + 1
    End If
    ...
    mHeader = mHeader + 1
    Print #1, PSTR("Chassis No. : " & Rstsale!ChassisNo, 45) & IIf(Rstsale!TOT_Per = 0, "", PSTR("Sub Total ", 23, , AlignRight) & ": " & PSTR(Txt(SubTotB), 11, 2))
    mHeader = mHeader + 1
    Print #1, PSTR("Engine No.  : " & Rstsale!EngineNo, 45) & IIf(Rstsale!TOT_Per = 0, "", PSTR(pubTOTCaption & " @ " & Format(Rstsale!TOT_Per, "0.00") & " %", 22, , AlignRight) & ": " & PSTR(Rstsale!Tot_Amt, 11, 2))
    mHeader = mHeader + 1
   ...
   ' Print #1, PSTR("Less  Fuel Amount", 65, , AlignRight) & " : " & PSTR(Rstsale!DieselAmt, 12, 2)
    ...
    Print #1, mChr17 & Rstsale!Inv_UName & " " & STR(Rstsale!Inv_UEntDt) & Space(((PageWidth * 1.7) - Len("") - Len(Rstsale!Inv_UName & " " & STR(Rstsale!Inv_UEntDt))) / 2) & "" & mChr18
    Print #1, mEject
    Close #1
    Open "C:\RepPrint.Bat" For Output As #1
'    If fob.FolderExists("c:\WinNt") Then
