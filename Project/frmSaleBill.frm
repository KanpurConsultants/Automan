...
Private Const Col_TaxForm As Byte = 17           ' Tax Per.
Private Const Col_TaxPer As Byte = 18           ' Tax Per.
Private Const Col_TaxAmt1 As Byte = 19          ' Tax. Amt.
Private Const Col_SatPer As Byte = 20           ' Tax Per.
Private Const Col_SatAmt As Byte = 21          ' Tax. Amt.
Private Const Col_ItemVal As Byte = 22         ' Item Value
Private Const Col_GodownCode As Byte = 23       ' Godown Code
Private Const Col_Godown As Byte = 24           ' Godown
Private Const Col_PartSrlNo As Byte = 25        ' Part Serial No
Private Const Col_PName As Byte = 26            ' Part Name
Private Const Col_LName As Byte = 27            ' Local Name
Private Const Col_MRPStkTB As Byte = 28         ' MRP Qty TB 'Current Stock Qty
Private Const Col_MRPStkTP As Byte = 29         ' MRP Qty TP
Private Const Col_TBStk As Byte = 30            ' Taxbale Qty
Private Const Col_TPStk As Byte = 31            ' Tax Paid Qty
Private Const Col_TBRate As Byte = 32           ' Taxbale Rate
Private Const Col_TPRate As Byte = 33           ' Tax Paid Rate
Private Const Col_Bin As Byte = 34              ' Bin
Private Const Col_LastRate As Byte = 35         ' Last Purchase Rate
Private Const Col_HPRate As Byte = 36           ' High Purchase Rate
Private Const Col_LPRate As Byte = 37           ' Low Purchase Rate
Private Const Col_PartGrade As Byte = 38        ' Part Grade (Used for Oil Item)
Private Const Col_EffectDate As Byte = 39       ' MRP Effective Date/TB Effective Date
Private Const Col_PurDocId As Byte = 40
Private Const Col_PurDate As Byte = 41
Private Const Col_TaxFormCode As Byte = 42
...
Private Sub Grid_Ini()
    With FGrid
        ...
        .ColAlignment(Col_TaxForm) = flexAlignLeftCenter
        .ColWidth(Col_TaxForm) = 600
...
'        Txt(NetSprAmt) = Format(Val(Txt(STotB)) + Val(Txt(STotATP)) + Val(Txt(TurnOverAmt)) + Val(Txt(SROff)), "0.00")
        ...
        Set Rst = New ADODB.Recordset
        Rst.CursorLocation = adUseClient
        Rst.Open "Select P.Part_Name,P.Local_Name,P.Unit,P.MRP,P.MRP_Effect_Dt,P.TB_SRate,P.TP_SRate,P.TB_Effect_Dt,P.Part_Grade ,P.Cur_MRP_TBStk,P.Cur_MRP_TPStk,P.Cur_TB_Stk ,P.Cur_TP_Stk ,P.Bin_Loca ,P.High_Pur_Rate, P.Low_Pur_Rate, Godown.God_Name," & cTrim(cMID("SP_Stock.DocID", "9", "5")) & " + " & cCStr(cTrim("Right(SP_Stock.DocID,8)")) & " As ChallIDDisp, " & cTrim(cMID("SP_Stock.Order_DocID", "9", "5")) & " + " & cCStr(cTrim("Right(SP_Stock.Order_DocID,8)")) & " As OrderIDDisp,SP_Stock.*,TaxForms.Form_Desc " & _
            ...
            " Left Join TaxForms on SP_Stock.TaxForm=TaxForms.Form_Code " & _
...
'                FGrid.AddItem i & Chr(9) & Rst!order_docid & Chr(9) & Rst!OrderIDDisp & Chr(9) & Rst!Order_Srl_No & Chr(9) & Rst!Part_No & Chr(9) & Rst!DocId & Chr(9) & Rst!ChallIDDisp & Chr(9) & Rst!Srl_No & Chr(9) & Rst!Unit & Chr(9) & Rst!MRPYN & Chr(9) & Rst!TaxYN & Chr(9) & Format(Rst!Qty_iss, "0.000") & Chr(9) & Format(Rst!Rate2, "0.00") & Chr(9) & Format(Rst!MRP_Rate2, "0.00") & Chr(9) & Format(mAmt, "0.00") & Chr(9) & Format(Rst!Disc_Per2, "0.00") & Chr(9) & Format(Rst!Disc_Amt2, "0.00") & Chr(9) & Format(Rst!Net_Amt2, "0.00") & Chr(9) & Rst!Godown & Chr(9) & Rst!God_Name & Chr(9) & Rst!Part_Name & Chr(9) & Rst!Local_Name & Chr(9) & Rst!Curstk & Chr(9) & Rst!MRPQty & Chr(9) & Rst!Cur_TB_Stk & Chr(9) & Rst!Cur_TP_Stk & Chr(9) & Rst!TB_SRate & Chr(9) & Rst!TP_SRate & Chr(9) & Rst!Bin_Loca & Chr(9) & " " & Chr(9) & Rst!high_pur_rate & Chr(9) & Rst!low_pur_rate & Chr(9) & Rst!Part_Grade & Chr(9) & Format(IIf(Rst!MRPYN = "Yes", Rst!MRP_Effect_Dt, Rst!TB_Effect_Dt), "dd/MMM/yyyy")
                ...
                With FGrid
                    ...
                    .TextMatrix(I, Col_TaxFormCode) = XNull(Rst!TaxForm)
                    If mVatYn = 1 Then
         ...
         If rsTaxPer.RecordCount > 0 Then
            ...
            FGrid.TextMatrix(FGrid.Row, Col_SatPer) = VNull(rsTaxPer!AddTaxPer)
                ...
                If UTrim(XNull(rsTaxPer!L_C)) = "LOCAL" Then
                   ...
                   Set rsTaxPer = GCn.Execute("Select TaxForms.Form_Desc, TaxFormSales, TaxForms.Tax_Per VatPer, TaxForms.AddTaxPer From Part_Grade Left Join TaxForms On Part_Grade.TaxFormSales = TaxForms.Form_Code Where PartGrade_Code='" & FGrid.TextMatrix(FGrid.Row, Col_PartGrade) & "'")
                   If rsTaxPer.RecordCount > 0 Then
                       FGrid.TextMatrix(FGrid.Row, Col_TaxFormCode) = XNull(rsTaxPer!TaxFormSales)
                       FGrid.TextMatrix(FGrid.Row, Col_TaxForm) = XNull(rsTaxPer!Form_Desc)
...
For I = 1 To FGrid.Rows - 1
    If FGrid.TextMatrix(I, Col_PNo) <> "" And Val(FGrid.TextMatrix(I, Col_Qty)) <> 0 Then
        ...
        GCn.Execute "Insert Into SP_Stock(" _
            ...
            & "Disc_Amt2,Amount2,Net_Amt2,Part_SrlNo,U_Name,U_EntDt,U_AE,TaxPer,TaxAmt, SatPer, SatAmt,PurDocNo,PurDocDate,TaxForm) " _
            ...
            " , " & Val(FGrid.TextMatrix(I, Col_DiscAmt)) & "," & Val(FGrid.TextMatrix(I, Col_Amt)) & "," & Val(FGrid.TextMatrix(I, Col_ItemVal)) & ",'" & FGrid.TextMatrix(I, Col_PartSrlNo) & "','" & pubUName & "'," & ConvertDate(PubServerDate) & ",'A'," & Val(FGrid.TextMatrix(I, Col_TaxPer)) & "," & Val(FGrid.TextMatrix(I, Col_TaxAmt1)) & "," & Val(FGrid.TextMatrix(I, Col_SatPer)) & "," & Val(FGrid.TextMatrix(I, Col_SatAmt)) & ",'" & FGrid.TextMatrix(I, Col_PurDocId) & "'," & ConvertDate(FGrid.TextMatrix(I, Col_PurDate)) & ",'" & FGrid.TextMatrix(I, Col_TaxFormCode) & "')"
...
If CDate(Txt(VDate)) > CDate("31/Mar/14") Then
    mQry = "SELECT Max(IsNull(Tf.PurSal_Ac_Code, DTF.PurSal_Ac_Code )) AS PurSal_Ac_Code, " & _
           ...
           "LEFT JOIN TaxFormsAc DTF ON DTF.Form_Code ='" & Txt(FormName).Tag & "' " & _
           "WHERE Invoice_DocId ='" & Txt(DocID) & "' " & _
...
'    Set RsLab = GCn.Execute(mQry)
    ...
    If RsSale.RecordCount > 0 Then
            ...
            If VNull(RsSale!RoundAmt) <> 0 Then
                ReDim Preserve LedgAry(UBound(LedgAry) + 1)
                LedgAry(UBound(LedgAry)).SubCode = XNull(rsCtrlAc!SprROff_Ac)
                LedgAry(UBound(LedgAry)).AmtDr = IIf(VNull(RsSale!RoundAmt) < 0, Abs(VNull(RsSale!RoundAmt)), 0)
                LedgAry(UBound(LedgAry)).AmtCr = IIf(VNull(RsSale!RoundAmt) > 0, Abs(VNull(RsSale!RoundAmt)), 0)
                LedgAry(UBound(LedgAry)).Narration = mNarr
            End If
    End If
    
    
    If Txt(DocType) = "Yes" Then
    ...
    If Val(Txt(NetSprAmt)) > 0 Then
        ...
        LedgAry(UBound(LedgAry)).AmtDr = Val(Txt(NetAmt))
        LedgAry(UBound(LedgAry)).AmtCr = 0
        LedgAry(UBound(LedgAry)).Narration = mNarr
    End If
    ...
    mResult = LedgerPost(mAddFlag, LedgAry, GCnFaS, Txt(DocID), CDate(Txt(VDate)), mCommNarr)
    If mResult <> 1 Then MsgBox "Error in Ledger Posting", vbOKOnly, "Validation"
