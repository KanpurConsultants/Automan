...
Begin VB.Form frmRequisition 
   ...
   Begin MSDataGridLib.DataGrid DgTaxForm 
      Height          =   2100
      Left            =   10620
      Negotiate       =   -1  'True
      TabIndex        =   132
...
Private Const Col_TaxForm As Byte = 14           ' Tax Per.
Private Const Col_TaxPer As Byte = 15           ' Tax Per.
Private Const Col_TaxAmt As Byte = 16           ' Tax Amt.
Private Const Col_SatPer As Byte = 17           ' Tax Per.
Private Const Col_SatAmt As Byte = 18           ' Tax Amt.
Private Const Col_ItemVal As Byte = 19          ' Item Value
Private Const Col_GodownCode As Byte = 20       ' Godown Code
Private Const Col_Godown As Byte = 21           ' Godown
Private Const Col_LubCat As Byte = 22           ' Lubricant Category
Private Const Col_RemWs As Byte = 23            ' Remark Workshop
Private Const Col_RemStores As Byte = 24        ' Remark Stores
Private Const Col_PName As Byte = 25            ' Part Name
Private Const Col_LName As Byte = 26            ' Local Name
Private Const Col_MRPStkTB As Byte = 27         ' MRP Stock TB
...
Private Const Col_DiscFact As Byte = 38         ' Discount Factor (Used for Disc%)
Private Const Col_EffectDate As Byte = 39       ' MRP Effective Date/TB Effective Date
Private Const Col_SrlNo As Byte = 40            ' SP_Stock SrlNo (DocID+SrlNo)
Private Const Col_PurDocId As Byte = 41
Private Const Col_PurDate As Byte = 42
Private Const Col_Supplier As Byte = 43

Private Const Col_DepItem As Byte = 44
Private Const Col_DepitemPer As Byte = 45
Private Const Col_DepCode As Byte = 46
Private Const Col_DepPer As Byte = 47
Private Const Col_DepAmt As Byte = 48
Private Const Col_InsuranceAmt As Byte = 49
Private Const Col_DiffPeried As Byte = 50
Private Const Col_TaxFormCode As Byte = 51
...
'        .ColWidth(Col_RemStores) = 1300
       ...
       .TextMatrix(0, Col_InsuranceAmt) = "Insurance Amt"
        ...
        .TextMatrix(0, Col_TaxFormCode) = "Tax Form Code"
        .ColAlignmentFixed(Col_TaxFormCode) = flexAlignRightCenter
...
On Error GoTo ELoop
    ...
    If Master.RecordCount > 0 Then
        ...
        Set Rst = GCn.Execute("Select P.Part_Name ,P.Local_Name ,P.Unit ,P.MRP ,P.TB_SRate ,P.TP_SRate ," _
            ...
            & "Emp_Mast.Emp_Name,Godown.God_Name, RateType.Description as RateTypeDescription, RateType.VariationPer,SP_Stock.*,h.Delivery_Date as SOldDate,TaxForms.Form_Desc   " _
...
Private Sub DGTaxForm_Click()
On Error GoTo ELoop
    DgTaxForm.Visible = False
    If RsTaxForm.RecordCount > 0 Then
        TxtGrid(0).TEXT = RsTaxForm!Name
        FGrid.TextMatrix(FGrid.Row, Col_TaxFormCode) = RsTaxForm!Code
        FGrid.TextMatrix(FGrid.Row, Col_TaxForm) = RsTaxForm!Name
    End If
    If TxtGrid(0).Visible = True Then TxtGrid(0).SetFocus
Exit Sub
ELoop:
...
Dim I As Byte
    ...
    Set RsTaxForm = GCn.Execute("Select Form_Code as Code, Form_Desc as Name From TaxForms Order By Form_Desc")
    Set DgTaxForm.DataSource = RsTaxForm
        ...
        For I = 1 To FGrid.Rows - 1
            If FGrid.TextMatrix(I, Col_PNo) <> "" And Val(FGrid.TextMatrix(I, Col_QtyReq)) <> 0 Then
                ...
                GCn.Execute "Insert Into SP_Stock(" _
                    ...
                    & "Purpose,Remark, RateType,U_Name,U_EntDt,U_AE,V_Rate,TaxForm,TaxPer,TaxAmt, SatPer, SatAmt, Sat_Yn,PurDocNo,PurDocDate, " & _
                    ...
                    & "'" & Purpose & "','" & FGrid.TextMatrix(I, Col_RemWs) + Space(20 - Len(FGrid.TextMatrix(I, Col_RemWs))) & "', '','" & pubUName & "'," & ConvertDate(PubServerDate) & ",'A'," & Val(FGrid.TextMatrix(I, Col_LastRate)) & ", " & Chk_Text(FGrid.TextMatrix(I, Col_TaxFormCode)) & "," & Val(FGrid.TextMatrix(I, Col_TaxPer)) & "," & Val(FGrid.TextMatrix(I, Col_TaxAmt)) & "," & Val(FGrid.TextMatrix(I, Col_SatPer)) & "," & Val(FGrid.TextMatrix(I, Col_SatAmt)) & ", " & IIf(mSatYn, 1, 0) & ",'" & FGrid.TextMatrix(I, Col_PurDocId) & "'," & ConvertDate(FGrid.TextMatrix(I, Col_PurDate)) & " , " & _
...
'            End If
    ...
    Case Col_Godown
        ...
        End If
    Case Col_TaxForm
        If RsTaxForm.RecordCount = 0 Or (RsTaxForm.EOF = True Or RsTaxForm.BOF = True) Or FGrid.TextMatrix(FGrid.Row, Col_TaxForm) = "" Then Exit Sub
        If FGrid.TextMatrix(FGrid.Row, Col_TaxForm) <> RsTaxForm!Name Then
            RsTaxForm.MoveFirst
            RsTaxForm.FIND "Name ='" & FGrid.TextMatrix(FGrid.Row, Col_TaxForm) & "'"
        End If
...
'                            FGrid_LeaveCell
            ...
            Case Col_Godown
                ...
                End If
            Case Col_TaxForm
                DGridTxtKeyDown DgTaxForm, TxtGrid, Index, RsTaxForm, KeyCode, True, 1, frmTaxForms, "frmTaxForms"
                If KeyCode = vbKeyReturn Then
                    If TxtGridLeave = True Then
                        GridTxtDown FGrid, TxtGrid, Index, KeyCode, TAddMode, IIf(FGrid.TextMatrix(FGrid.Row, Col_PartGrade) = PubPartGrade_Lub, Col_LubCat, IIf(left(FGrid.TextMatrix(FGrid.Row, Col_Purpose), 1) = "W", Col_RemStores, Col_PNo)), 1, IIf(FGrid.TextMatrix(FGrid.Row, Col_PartGrade) = PubPartGrade_Lub, Col_LubCat, IIf(left(FGrid.TextMatrix(FGrid.Row, Col_Purpose), 1) = "W", Col_RemWs, 0))
                    End If
...
Select Case FGrid.Col
    ...
    Case Col_Godown
        ...
        End If
    Case Col_TaxForm
        If DgTaxForm.Visible = True Then
            DGridTxtKeyPress TxtGrid, Index, RsTaxForm, KeyAscii, "Name"
        End If
...
Select Case FGrid.Col
    ...
    Case Col_Godown
        ...
        End If
    Case Col_TaxForm
        If KeyCode <> 13 And DgTaxForm.Visible = False Then
            TxtGrid_KeyDown Index, GridKey, 0
            DGridTxtKeyPress TxtGrid, Index, RsTaxForm, KeyCode, "Name", True
        End If
...
Private Sub TxtGridValid_TaxForm()

    If RsTaxForm.RecordCount = 0 Or (RsTaxForm.EOF = True Or RsTaxForm.BOF = True) Or TxtGrid(0).TEXT = "" Then
        FGrid.TextMatrix(FGrid.Row, Col_TaxFormCode) = ""
        FGrid.TextMatrix(FGrid.Row, Col_TaxForm) = ""
    Else
        FGrid.TextMatrix(FGrid.Row, Col_TaxFormCode) = RsTaxForm!Code
        FGrid.TextMatrix(FGrid.Row, Col_TaxForm) = RsTaxForm!Name
    End If
...
Private Sub SetMaxLength()
    Select Case FGrid.Col
        ...
        Case Col_LubCat, Col_Purpose, Col_RemWs, Col_Godown, Col_TaxForm
