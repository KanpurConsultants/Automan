...
If RstVType.RecordCount > 0 Then
    ...
    Select Case RstVType!NCat
        ...
        Case "SXPIC", "SXPIR"
            frmPurBill.Show
            frmPurBill.SEARCHBACK FGrid1.TextMatrix(FGrid1.Row, 12)
        Case "SYSIC", "SYSIR"
            frmSaleBill.Show
            frmSaleBill.SEARCHBACK FGrid1.TextMatrix(FGrid1.Row, 12)
        Case "W_LIC", "W_LIR"
            mDocId = GCn.Execute("Select IsNull(Max(DocID),'') from Job_Card Where DocId_InvLab='" & FGrid1.TextMatrix(FGrid1.Row, 12) & "'").Fields(0).Value
            If mDocId <> "" Then
                frmJobClose.Show
                frmJobClose.SEARCHBACK mDocId
            End If
        Case "W_SIC", "W_SIR"
            mDocId = GCn.Execute("Select IsNull(Max(DocID),'') from Job_Card Where DocId_InvSpr='" & FGrid1.TextMatrix(FGrid1.Row, 12) & "'").Fields(0).Value
            If mDocId <> "" Then
                frmJobClose.Show
                frmJobClose.SEARCHBACK mDocId
            End If
        ...
        Case "V_PB"
            frmVehPur.Show
            frmVehPur.mVType = "V_PB"
            frmVehPur.SEARCHBACK FGrid1.TextMatrix(FGrid1.Row, 12)
        Case "V_SB"
            frmVehSale.Show
