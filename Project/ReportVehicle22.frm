...
Begin VB.Form ReportVehicle 
   ...
   Begin VB.CommandButton BTNPRINT 
      BackColor       =   &H00C0FFFF&
      Caption         =   "&Print"
      DownPicture     =   "ReportVehicle22.frx":0000
      BeginProperty Font 
         Name            =   "Arial"
         Size            =   9
         Charset         =   0
   ...
   Begin VB.CommandButton BTNEXIT 
      BackColor       =   &H00C0FFFF&
      Caption         =   "E&xit"
      DownPicture     =   "ReportVehicle22.frx":3132
      BeginProperty Font 
         Name            =   "Arial"
         Size            =   9
         Charset         =   0
...
Private Const VehSumModel As Byte = 13
    ...
    Case List3
        Select Case GRepFormName
            Case PurchReg
                ...
                Set mListItem = ListView_Items(ListView, TxtGrid, Index, ListArray, 2)
            Case VehSaleReg
                ListArray = Array("SalesManWise", "PartyWise", "CityWise", "FinancierGrp", "FinancierName", "FormType", "All")
...
Case ChsRecReg    'vijay Vehicle 16/11/02
        ...
        Grid3Sql = "select '' AS O,SubGroup.NAME as Party_Name,SubGroup.SubCode as Code from SubGroup " & _
            "left join [" & PubVFADataPath & "].AcGroup on SubGroup.GroupCode=AcGroup.GroupCode " & _
...
Case VehSaleReg    'vijay Vehicle 16/11/02
        With FGrid
            ...
            .TextMatrix(List2, 0) = "Cancelled(Y/N)": .RowHeight(List2) = GridRowHeight
            .TextMatrix(List3, 0) = "Type": .RowHeight(List3) = GridRowHeight

            .TextMatrix(Date1, 1) = PubStartDate
            .TextMatrix(Date2, 1) = PubLoginDate
            .TextMatrix(List1, 1) = "Summary"
            .TextMatrix(List2, 1) = "Yes"
            .TextMatrix(List3, 1) = "PartyWise"
        End With
        ...
        GridInitialise 4, Grid4Sql
Case VehInTrans, AddFitRep, VehSumModel  'vijay Vehicle 16/11/02
        With FGrid
            .TextMatrix(Date1, 0) = "From Date": .RowHeight(Date1) = GridRowHeight
            .TextMatrix(Date2, 0) = "UpTo Date": .RowHeight(Date2) = GridRowHeight
...
Case DelChaReg    'vijay Vehicle
    ...
    Case PurchReg    'vijay WKS 16/11/02
        With FGrid
            .TextMatrix(Date1, 0) = "From Date": .RowHeight(Date1) = GridRowHeight
            .TextMatrix(Date2, 0) = "UpTo Date": .RowHeight(Date2) = GridRowHeight
            .TextMatrix(List1, 0) = "All/Taxable/TPaid": .RowHeight(List1) = GridRowHeight
...
Case ChsRecReg, PurchReg, VehInTrans, VehQuot, AddFitRep, VehSumModel, VehInTrans, VehSalePurRep
    For I = 1 To rpt.FormulaFields.Count
        Select Case UCase(rpt.FormulaFields(I).FormulaFieldName)
            Case UCase("DATEBETWEEN")
                rpt.FormulaFields(I).Text = "'From :'+ '" & Format(FGrid.TextMatrix(Date1, 1), "dd/mmm/yyyy") & "' + ' To ' + '" & Format(FGrid.TextMatrix(Date2, 1), "dd/mmm/yyyy") & "'"
...
Dim mQry As String, CondStr As String
    ...
    CondStr = " where Veh_Quot.V_Date >= #" & Format(FGrid.TextMatrix(Date1, 1), "dd/MMM/yyyy") & "# and Veh_Quot.V_Date <= #" & Format(FGrid.TextMatrix(Date2, 1), "dd/MMM/yyyy") & "# "
...
'   If IsNotBlank(List1, FGrid.TextMatrix(List2, 0)) = False Then RepPrint = False: Exit Sub
    ...
    CondStr = " where Veh_Stock.Chassis_RctDate >= #" & Format(FGrid.TextMatrix(Date1, 1), "dd/MMM/yyyy") & "# and Veh_Stock.Chassis_RctDate <= #" & Format(FGrid.TextMatrix(Date2, 1), "dd/MMM/yyyy") & "# "
...
Dim mQry$, mQry1$, CondStr$
    ...
    CondStr = " and (VStk.InDate <= #" & Format(FGrid.TextMatrix(Date1, 1), "dd/MMM/yyyy") & "#) "
    
    If Check1(1).Value = Unchecked Then CondStr = CondStr & " and (mid(VStk.Pur_DocId,3,1) in (" & GridString1 & ") or VStk.Chassis_RctDivCode in (" & GridString1 & "))"
    ...
    mQry = "SELECT 1 as Qty,VStk.Chassis_RctDocNo,VStk.Model,VStk.ChassisNo,VStk.EngineNo,ColMast.Col_Desc,VStk.Pur_DocId,VStk.Pur_VDate," & _
        "VStk.PBILL_NO,VStk.PBILL_DATE,S.Name AS SupplierName,VStk.TAX_YN,VStk.Rate,VStk.VRate,VStk.Sal_DocID,VStk.Sal_VDate," & _
        ...
        "LEFT JOIN ColMast ON VStk.Colour_Code = ColMast.Col_Code) "
        
    mQry1 = "SELECT 1 as Qty,VStk.Chassis_RctDocNo,VStk.Model,VStk.ChassisNo,VStk.EngineNo,ColMast.Col_Desc,VStk.Pur_DocId,VStk.Pur_VDate," & _
        "VStk.PBILL_NO,VStk.PBILL_DATE,S.Name AS SupplierName,VStk.TAX_YN,VStk.Rate,VStk.VRate,VStk.Sal_DocID,VStk.Sal_VDate," & _
        "'' as PartyName " & _
...
'        mQRY1 = mQRY1 & " WHERE (IsNull(VStk.Sal_VDate) )"
        mQry = mQry & " WHERE (Not IsNull(VO.Inv_Date) and VO.Inv_Date< #" & Format(FGrid.TextMatrix(Date1, 1), "dd/MMM/yyyy") & "#)"
        mQry1 = mQry1 & " WHERE (IsNull(VStk.Sal_DocId) or VStk.Sal_DocID='' )"
    End If
    If FGrid.TextMatrix(List1, 1) = "W/O Sale" Then
'        mQRY = mQRY & " WHERE (VStk.Sal_VDate> #" & Format(FGrid.TextMatrix(Date1, 1), "dd/MMM/yyyy") & "#)"
'        mQRY1 = mQRY1 & " WHERE (IsNull(VStk.Sal_VDate) )"
        mQry = mQry & " WHERE (VO.Inv_Date> #" & Format(FGrid.TextMatrix(Date1, 1), "dd/MMM/yyyy") & "#)"
        mQry1 = mQry1 & " WHERE (IsNull(VStk.Sal_DocID) or VStk.Sal_DocID='' )"
    End If
...
Dim mQry$, mQry1$, CondStr$
    ...
    CondStr = " and VStk.Pur_VDate <= #" & Format(FGrid.TextMatrix(Date1, 1), "dd/MMM/yyyy") & "# "
...
'    If IsNotBlank(Date2, FGrid.TextMatrix(Date2, 0)) = False Then RepPrint = False: Exit Sub
    ...
    mQry = "SELECT 1 as Qty,VP1.BMS_CATEGORY,VStk.Godown,Godown.God_Name,VStk.Model,VStk.ChassisNo,VStk.EngineNo,ColMast.Col_Desc,VStk.Chassis_RctSiteCode," & _
        ...
        "VStk.InDate,VStk.TAX_YN,VStk.VRate,VStk.Sal_DocID,VStk.Sal_VDate " & _
        "FROM ((((((Veh_Stock as VStk LEFT JOIN Veh_Order as VO ON VStk.Sal_DocId=VO.Inv_DocId) " & _
...
'    If IsNotBlank(List1, FGrid.TextMatrix(List2, 0)) = False Then RepPrint = False: Exit Sub
    ...
    CondStr = " WHERE  V_O.Inv_Date >= #" & Format(FGrid.TextMatrix(Date1, 1), "dd/MMM/yyyy") & "# and V_O.Inv_Date <= #" & Format(FGrid.TextMatrix(Date2, 1), "dd/MMM/yyyy") & "# "
    
    If Check1(1).Value = Unchecked Then CondStr = CondStr & " and mid(V_O.Inv_DocId,3,1) in (" & GridString1 & ")"
...
'    mQRY = "SELECT SG.Name, SG.Add1, SG.Add2, SG.Add3, V_O.ord_Date, V_S.ChassisNo, V_S.MODEL," & _
    ...
    mQry = "SELECT SG.Name, SG.Add1, SG.Add2, SG.Add3, V_O.ord_Date, V_S.ChassisNo, V_S.MODEL," & _
           " V_S.PBILL_NO AS TelcoInvNo, V_S.PBILL_DATE AS TelcoDate, V_O.MARGINE,(V_O.Net_Amount - V_O.VRate) As Difference, " & _
           " V_S.VRATE,iif(ContractFinance.FinCatg=0,ContractFinance.FinName,'')AS finName,V_O.Interest,V_O.RTO,V_o.margine as stdmargine,Rect.V_Type,Rect.V_Date,Rect.V_No,Rect.AMOUNT,Rect.DrCr,SubGroupType.Description,City.CityName,V_O.Transport,V_O.Inv_No,V_O.Inv_Date,V_O.Net_Amount as SaleRate,V_O.Inv_DocId,SG.NamePrefix " & _
...
'   If IsNotBlank(List1, FGrid.TextMatrix(List2, 0)) = False Then RepPrint = False: Exit Sub
    ...
    If FGrid.TextMatrix(List1, 1) = "Delivered" Then
        CondStr = " where VO.DelCh_DT >= #" & Format(FGrid.TextMatrix(Date1, 1), "dd/MMM/yyyy") & "# and VO.DelCh_DT <= #" & Format(FGrid.TextMatrix(Date2, 1), "dd/MMM/yyyy") & "# "
    Else
        CondStr = " where VO.Inv_Date >= #" & Format(FGrid.TextMatrix(Date1, 1), "dd/MMM/yyyy") & "# and VO.Inv_Date <= #" & Format(FGrid.TextMatrix(Date2, 1), "dd/MMM/yyyy") & "# "
    End If
      
    If Check1(1).Value = Unchecked Then CondStr = CondStr & " and mid(VO.Inv_DocId,3,1) in (" & GridString1 & ")"
    ...
    If FGrid.TextMatrix(List1, 1) = "Delivered" Then
           CondStr = CondStr & " and Not IsNull(VO.DelCh_DT) "
    ElseIf FGrid.TextMatrix(List1, 1) = "Pending" Then
           CondStr = CondStr & " and IsNull(VO.DelCh_DT) "
    End If
...
Dim mQry As String, CondStr As String
    ...
    CondStr = " where Veh_Purch2.V_TYPE ='V_SB'  and  Veh_Order.Inv_Date >= #" & Format(FGrid.TextMatrix(Date1, 1), "dd/MMM/yyyy") & "# and Veh_Order.Inv_Date <= #" & Format(FGrid.TextMatrix(Date2, 1), "dd/MMM/yyyy") & "# "

    If Check1(1).Value = Unchecked Then CondStr = CondStr & " and mid(Veh_Order.Inv_DocId,3,1) in (" & GridString1 & ")"
...
'    If Check1(2).Value = Unchecked Then GridString2 = FillString(GridRow2, 2, 1): If RepPrint = False Then Exit Sub
    ...
    mOpQry = "SELECT M.Div_Code,D.Div_SName,MG.ModelGrp_Name,MC.ModelCat_Name,VStk.Model,1 as MonthOpen, 0 AS MonthPurTL, 0 as MonthPurDL, 0 as PurDay, 0 AS MonthSal, 0 as SalDay, '' as AreaName " & _
        ...
        " and (isnull(VStk.Sal_VDate) or format(VStk.Sal_VDate,'yyyymm')< '" & Format(FGrid.TextMatrix(Date1, 1), "yyyymm") & "')"
    
    mQry = "SELECT M.Div_Code,D.Div_SName,MG.ModelGrp_Name,MC.ModelCat_Name,VStk.Model,0 as MonthOpen, " & _
        " iif(VStk.Pur_VDate < #" & Format(FGrid.TextMatrix(Date1, 1), "dd/MMM/yyyy") & "# and VP.OBNO<>'',1,0) AS MonthPurTL, " & _
        " iif(VStk.Pur_VDate < #" & Format(FGrid.TextMatrix(Date1, 1), "dd/MMM/yyyy") & "# and VP.OBNO<>'',0,1) AS MonthPurDL, " & _
        " iif(VStk.Pur_VDate = #" & Format(FGrid.TextMatrix(Date1, 1), "dd/MMM/yyyy") & "#,1,0) AS PurDay, " & _
        ...
        " Left join Division D on M.Div_Code=D.Div_Code " & _
        " where (VStk.Pur_DocId='' and " & ChasDivCond & " Format(Chassis_RctDate,'YYYYMM')='" & Format(FGrid.TextMatrix(Date1, 1), "YYYYMM") & "' and Chassis_RctDate <= #" & Format(FGrid.TextMatrix(Date1, 1), "dd/MMM/yyyy") & "#) " & _
        " or (" & PurDivCond & " format(VStk.Pur_VDate,'YYYYMM')= '" & Format(FGrid.TextMatrix(Date1, 1), "YYYYMM") & "' and VStk.Pur_VDate <= #" & Format(FGrid.TextMatrix(Date1, 1), "dd/MMM/yyyy") & "#)"
...
'Cat_Code  ModelCat_Code    ModelCat_Name
    mQry1 = "SELECT M.Div_Code,D.Div_SName,MG.ModelGrp_Name,MC.ModelCat_Name,VO.Model,0 as MonthOpen, 0 as MonthPurTL, 0 as MonthPurDL, 0 as PurDay," & _
        " iif(VO.Inv_Date< #" & Format(FGrid.TextMatrix(Date1, 1), "dd/MMM/yyyy") & "#,1,0) AS MonthSal, " & _
        " iif(VO.Inv_Date = #" & Format(FGrid.TextMatrix(Date1, 1), "dd/MMM/yyyy") & "#,1,0) AS SalDay, Area.AreaName " & _
        ...
        "' and VO.Inv_Date <= #" & Format(FGrid.TextMatrix(Date1, 1), "dd/MMM/yyyy") & "#"
    ...
    mQryArea = "SELECT Distinct Area.AreaName " & _
        ...
        " Left join Division D on M.Div_Code=D.Div_Code " & _
        " where " & SalDivCond & _
        " VO.Inv_Date = #" & Format(FGrid.TextMatrix(Date1, 1), "dd/MMM/yyyy") & "# Order By Area.AreaName"
...
'    If Check1(2).Value = Unchecked Then GridString2 = FillString(GridRow2, 2, 1): If RepPrint = False Then Exit Sub
    ...
    mOpQry = "SELECT M.Div_Code,MG.ModelGrp_Name,MC.ModelCat_Name,VStk.Model,1 as DayOpen, 0 AS MonthPur, 0 as PurDay, 0 AS MonthSal, 0 as SalDay, '' as AreaName " & _
        ...
        " Left join Model_Grp MG on M.Grp_Code=MG.ModelGrp_Code) " & _
        " Left join Model_Cat MC on M.Cat_Code=MC.ModelCat_Code " & _
        " where (VStk.Pur_DocId='' and " & ChasDivCond & " Chassis_RctDate<#" & Format(FGrid.TextMatrix(Date1, 1), "dd/MMM/yyyy") & "#) " & _
        " or (" & PurDivCond & " VStk.Pur_VDate < #" & Format(FGrid.TextMatrix(Date1, 1), "dd/MMM/yyyy") & "#) " & _
        " and (isnull(VStk.Sal_VDate) or VStk.Sal_VDate<= #" & Format(FGrid.TextMatrix(Date1, 1), "dd/MMM/yyyy") & "#)"
...
'    If IsNotBlank(Date2, FGrid.TextMatrix(Date2, 0)) = False Then RepPrint = False: Exit Sub
    ...
    CondStr = " where Pur_VDate <= #" & Format(FGrid.TextMatrix(Date1, 1), "dd/MMM/yyyy") & "#"
    ...
    mQry = "SELECT Model.Model_Desc,Veh_F.MODEL," & _
           ...
           "iif( Pur_VDate >= #" & Format("01/" & "Apr/" & Format(PubStartDate, "yyyy"), "dd/MMM/yyyy") & "# AND Pur_VDate <= #" & Format("30/" & "Apr/" & Format(PubStartDate, "yyyy"), "dd/MMM/yyyy") & "#,1,0) AS AprPurch, " & _
           "iif( Pur_VDate >= #" & Format("01/" & "May/" & Format(PubStartDate, "yyyy"), "dd/MMM/yyyy") & "# AND Pur_VDate <= #" & Format("31/" & "May/" & Format(PubStartDate, "yyyy"), "dd/MMM/yyyy") & "#,1,0) AS MayPurch, " & _
           "iif( Pur_VDate >= #" & Format("01/" & "Jun/" & Format(PubStartDate, "yyyy"), "dd/MMM/yyyy") & "# AND Pur_VDate <= #" & Format("30/" & "Jun/" & Format(PubStartDate, "yyyy"), "dd/MMM/yyyy") & "#,1,0) AS JunPurch, " & _
           "iif( Pur_VDate >= #" & Format("01/" & "Jul/" & Format(PubStartDate, "yyyy"), "dd/MMM/yyyy") & "# AND Pur_VDate <= #" & Format("31/" & "Jul/" & Format(PubStartDate, "yyyy"), "dd/MMM/yyyy") & "#,1,0) AS JulPurch, " & _
           "iif( Pur_VDate >= #" & Format("01/" & "Aug/" & Format(PubStartDate, "yyyy"), "dd/MMM/yyyy") & "# AND Pur_VDate <= #" & Format("31/" & "Aug/" & Format(PubStartDate, "yyyy"), "dd/MMM/yyyy") & "#,1,0) AS AugPurch, " & _
           "iif( Pur_VDate >= #" & Format("01/" & "Sep/" & Format(PubStartDate, "yyyy"), "dd/MMM/yyyy") & "# AND Pur_VDate <= #" & Format("30/" & "Sep/" & Format(PubStartDate, "yyyy"), "dd/MMM/yyyy") & "#,1,0) AS SepPurch, " & _
           "iif( Pur_VDate >= #" & Format("01/" & "Oct/" & Format(PubStartDate, "yyyy"), "dd/MMM/yyyy") & "# AND Pur_VDate <= #" & Format("31/" & "Oct/" & Format(PubStartDate, "yyyy"), "dd/MMM/yyyy") & "#,1,0) AS octPurch, " & _
           "iif( Pur_VDate >= #" & Format("01/" & "Nov/" & Format(PubStartDate, "yyyy"), "dd/MMM/yyyy") & "# AND Pur_VDate <= #" & Format("30/" & "Nov/" & Format(PubStartDate, "yyyy"), "dd/MMM/yyyy") & "#,1,0) AS NovPurch, " & _
           "iif( Pur_VDate >= #" & Format("01/" & "Dec/" & Format(PubStartDate, "yyyy"), "dd/MMM/yyyy") & "# AND Pur_VDate <= #" & Format("31/" & "Dec/" & Format(PubStartDate, "yyyy"), "dd/MMM/yyyy") & "#,1,0) AS DecPurch, " & _
           "iif( Pur_VDate >= #" & Format("01/" & "Jan/" & Format(PubEndDate, "yyyy"), "dd/MMM/yyyy") & "# AND Pur_VDate <= #" & Format("31/" & "Jan/" & Format(PubEndDate, "yyyy"), "dd/MMM/yyyy") & "#,1,0) AS JanPurch, " & _
           "iif( Pur_VDate >= #" & Format("01/" & "Feb/" & Format(PubEndDate, "yyyy"), "dd/MMM/yyyy") & "# AND Pur_VDate <= #" & Format(fxLastDay(Format("27/" & "Feb/" & Format(PubEndDate, "yyyy"), "dd/MMM/yyyy")) & "/Feb/" & Format(PubEndDate, "yyyy"), "dd/MMM/yyyy") & "#,1,0) AS FebPurch, " & _
           "iif( Pur_VDate >= #" & Format("01/" & "Mar/" & Format(PubEndDate, "yyyy"), "dd/MMM/yyyy") & "# AND Pur_VDate <= #" & Format("31/" & "Mar/" & Format(PubEndDate, "yyyy"), "dd/MMM/yyyy") & "#,1,0) AS MarPurch " & _
...
On Error GoTo ELoop
Dim mQry$, mQry1$, mQry2$, CondStr As String
    ...
    If Check1(2).Value = Unchecked Then CondStr = CondStr & " and VS.Model in (" & GridString2 & ")"
    If Check1(3).Value = Unchecked Then CondStr = CondStr & " and left(VS.Pur_DocId,1) in (" & GridString3 & ")"
 '(IsNull(VStk.Sal_DocId) or VStk.Sal_DocID='' )
      'modishekhar
    mQry = "SELECT VS.Model,1 as Opening,0 as Purchase,0 as Sale,0 as LastPurch,0 as LastSale " & _
        " from Veh_Stock as VS left join Veh_Order as VO on VS.Sal_DocID=VO.Inv_DocID " & _
        " WHERE VS.Pur_VDate <= #" & Format(FGrid.TextMatrix(Date1, 1), "dd/MMM/yyyy") & _
        "# And ((IsNull(VS.Sal_DocId) or VS.Sal_DocID='') or VO.Inv_Date > #" & Format(FGrid.TextMatrix(Date1, 1), "dd/MMM/yyyy") & "#) "
        

    mQry1 = "SELECT VS.Model,0 as Opening,1 as Purchase,0 as Sale," _
        & "iif(VS.Pur_VDate = #" & Format(FGrid.TextMatrix(Date2, 1), "dd/MMM/yyyy") & "#,1,0) as LastPurch,0 as LastSale " _
        & " from Veh_Stock as VS " _
        & " WHERE (VS.Pur_VDate >= #" & Format(FGrid.TextMatrix(Date1, 1), "dd/MMM/yyyy") & _
        "# And VS.Pur_VDate <= #" & Format(FGrid.TextMatrix(Date2, 1), "dd/MMM/yyyy") & "#)"

    mQry2 = "SELECT VS.Model,0 as Opening,0 as Purchase,1 as Sale,0 as LastPurch," _
        & "iif(VO.Inv_Date = #" & Format(FGrid.TextMatrix(Date2, 1), "dd/MMM/yyyy") & "#,1,0) as LastSale " _
        & " from Veh_Stock as VS left join Veh_Order as VO on VS.Sal_DocID=VO.Inv_DocID " _
        & " WHERE (VO.Inv_Date >= #" & Format(FGrid.TextMatrix(Date1, 1), "dd/MMM/yyyy") & _
        "# And VO.Inv_Date <= #" & Format(FGrid.TextMatrix(Date2, 1), "dd/MMM/yyyy") & "#)"


    mQry = mQry & CondStr & " Union All " & mQry1 & CondStr & " Union All " & mQry2 & CondStr
...
'    If IsNotBlank(List1, FGrid.TextMatrix(List2, 0)) = False Then RepPrint = False: Exit Sub
    ...
    Select Case FGrid.TextMatrix(List3, 1)
        Case "PartyWise"
    ...
    mQry = "SELECT VO.Inv_DocId, VO.Inv_Date, VO.OrdDocId,VO.Ord_Date,SG.NamePrefix,Emp_Mast.Emp_Name as Name, SG.FPrefix, SG.FName, SG.Add1, SG.Add2, SG.Add3,'' as CityName, " & _
           ...
           "VO.Form_Code,TF.Form_Desc,FinGroup.FinGrpName,ContractFinance.FinName,VO.Fin_Amt,VO.MISC_INFO" & _
           ...
           GoTo NXT
        Case Else
        CondStr = CondStr & " order by VO.Inv_Date,VO.Inv_DocId"
      End Select
...
NXT:
    ...
    End If
    If FGrid.TextMatrix(List3, 1) = "All" And FGrid.TextMatrix(List1, 1) = "Summary" Then
        RepName = "VehSaleRegSumAll"
    End If
    If FGrid.TextMatrix(List3, 1) = "All" And FGrid.TextMatrix(List1, 1) = "Detailed" Then
...
'    If IsNotBlank(Date1, FGrid.TextMatrix(Date1, 1)) = False Then RepPrint = False: Exit Sub
    ...
    CondStr = " where VP1.V_Date <= #" & Format(FGrid.TextMatrix(Date2, 1), "dd/MMM/yyyy") & "# "
   ...
   mQry = "SELECT VP1.V_NO, VP1.V_Date, VP1.PBILL_NO, VP1.PBILL_DATE," & _
          ...
          "(#" & Format(FGrid.TextMatrix(Date2, 1), "dd/MMM/yyyy") & "# - VP1.V_Date) AS TDay, " & _
    ...
    mQry = mQry + CondStr + "AND Subgroup.nature='Supplier' And IsNull(VStk.InDate)"
...
FormulaStr4 = ""
    ...
    If Check1(4).Value = Unchecked Then GridString4 = FillString(GridRow4, 4, 1): If RepPrint = False Then Exit Sub
     ...
     CondStr = "  VP1.V_Date >= #" & Format(FGrid.TextMatrix(Date1, 1), "dd/MMM/yyyy") & "# and VP1.V_Date <= #" & Format(FGrid.TextMatrix(Date2, 1), "dd/MMM/yyyy") & "#"
...
'    If FGrid.TextMatrix(List1, 1) = "All" Then 'CondStr = CondStr & " and isnull(Job_Card.JobCloseDate)"
      ...
      mQry = "SELECT SG.Name,BMS.BMS_Name,ColMast.Col_Desc,VP1.DocID,VP1.V_No,VP1.PBILL_NO AS BillNo,VP1.PBILL_DATE AS BillDate," & _
            "VStk.MODEL,VStk.ChassisNo,VStk.EngineNo,VStk.INDATE,VP1.V_Date As PurchDate,VP1.V_NO AS MemoNo,VP1.Exsice," & _
            "VP1.AMOUNT,VP1.Addition,VP1.Deduction,((#" & Format(FGrid.TextMatrix(Date2, 1), "dd/MMM/yyyy") & "#) - VP1.PBILL_DATE) AS AgeDays," & _
    ...
    End If
    If FGrid.TextMatrix(List1, 1) = "Taxable" Then
        mQry = mQry & "iif(VStk.TAX_YN=1,VP1.TAX_Amt,1) AS TaxAmt "
    End If
    If FGrid.TextMatrix(List1, 1) = "TaxPaid" Then
        mQry = mQry & "iif(VStk.TAX_YN=0,VP1.TAX_Amt,1) AS TaxAmt "
    End If
    mQry = mQry & " FROM ((((Veh_Purch1 as VP1 LEFT JOIN Veh_Stock as VStk ON VP1.DocID = VStk.Pur_DocId)) " & _
    ...
    If FGrid.TextMatrix(List3, 1) = "Voucher No" Then
        mQry = mQry & "Order By VP1.V_Date and VP1.Docid"
    Else
...
Dim mQry As String, CondStr As String, CondStr1 As String
    ...
    CondStr = " WHERE VStk.Pur_vDate >= #" & Format(FGrid.TextMatrix(Date1, 1), "dd/MMM/yyyy") & "# and VStk.Pur_vDate <= #" & Format(FGrid.TextMatrix(Date2, 1), "dd/MMM/yyyy") & "# "
    ...
    If FGrid.TextMatrix(List1, 1) = "V-No" Then
     ...
     mQry = "SELECT " & _
                ...
                "IIf(SubGroup.Nature='Supplier',SubGroup.name,'') AS P_PartyName, VStk.MODEL," & _
                "VStk.ChassisNo, VStk.EngineNo, RIGHT(VO.Inv_docId,13) AS InvNo," & _
                "VO.Inv_Date, VO.TAX_Amt AS SaleTax, VO.Surcharge_Amt AS SaleSurAmt," & _
                "VO.Net_AMOUNT AS Net_InvVal,VStk.Vrate, IIf(S1.Nature='Customer',S1.name,'') AS S_PartyName" & _
            " FROM (((((Veh_Stock as Vstk LEFT JOIN Veh_Order as VO ON VStk.Sal_DocId = VO.inv_DocId)" & _
                "LEFT JOIN SubGroup ON VStk.PartyCode = SubGroup.SubCode)" & _
                "LEFT JOIN SubGroup AS S1 ON VO.PartyCode=S1.SubCode) " & _
                "LEFT JOIN City ON SubGroup.CityCode = City.CityCode)" & _
                "LEFT JOIN Veh_Purch1 as VP1 ON VStk.Pur_DocId = VP1.DocID)"
                mQry = mQry + CondStr & "  order by RIGHT(VStk.Pur_DocID,13)ASC"
...
Dim mQry As String, CondStr As String, CondStr1 As String
    ...
    CondStr = " where Veh_Order.Ord_Date >= #" & Format(FGrid.TextMatrix(Date1, 1), "dd/MMM/yyyy") & "# and Veh_Order.Ord_Date <= #" & Format(FGrid.TextMatrix(Date2, 1), "dd/MMM/yyyy") & "# "
    ...
    If FGrid.TextMatrix(List1, 1) = "All" Then 'CondStr = CondStr & " and isnull(Job_Card.JobCloseDate)"
         mQry = "SELECT Veh_Order.OrdDocId,Veh_Order.Ord_Date AS VehOrdDate, right(Veh_Order.OrdDocId,13) AS VehOrdNo, veh_order.Model," & _
            ...
            "(Rect LEFT JOIN [" & PubVFADataPath & "].Voucher_Type ON Rect.V_Type = Voucher_Type.V_Type) ON Veh_Order.OrdDocId = Rect.ord_DocId)" & _
    ...
    End If
    If FGrid.TextMatrix(List1, 1) = "Pending" Then
        CondStr = CondStr & " and IsNull(Veh_Order.Inv_Date) Or Veh_Order.Inv_Date > #" & Format(FGrid.TextMatrix(Date2, 1), "dd/MMM/yyyy") & "#"
    ...
    If FGrid.TextMatrix(List1, 1) = "Supplied" Then
        CondStr = CondStr & " and (Not IsNull(Veh_Order.Inv_Date) Or Veh_Order.Inv_Date > #" & Format(FGrid.TextMatrix(Date2, 1), "dd/MMM/yyyy") & "#)"
        mQry = "SELECTVeh_Order.OrdDocId,Veh_Order.Ord_Date AS VehOrdDate, Veh_Order.Ord_No AS VehOrdNo, veh_order.Model," & _
            "SubGroup.Name AS PartyName,SubGroup.NamePrefix,City.CityName,SubGroup.Add1,SubGroup.Add2, Rect.V_Date AS RectDate, Rect.V_No AS RectNo,FinName," & _
            "Rect.AMOUNT AS RectAmt, Rect.Narration AS RectNarr, Rect.Narration1 AS RectNarr1," & _
            "ColMast.Col_Desc AS Colour, Veh_Order.Inv_No,Veh_Stock.Pbill_No,Veh_Stock.Pbill_Date,Veh_Order.vrate," & _
            "Veh_Order.Inv_Date AS InvDate, Veh_Order.Net_Amount  AS InvoiceAmt,Veh_Order.Inv_DocId" & _
